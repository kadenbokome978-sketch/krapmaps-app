      ...
      <div style={{ borderRadius:D.phone?12:20, padding:D.phone?"14px 14px":"20px 22px", position:"relative", overflow:"hidden", background:"linear-gradient(135deg,#120020 0%,#1E0035 50%,#120020 100%)", border:`1px solid ${C.pink}30` }}>
        ...
        <div style={{ position:"relative" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              <div style={{ color:C.text, fontSize:D.phone?20:28, fontWeight:900, fontFamily:"'Sora', sans-serif", letterSpacing:"-0.02em", lineHeight:1.1 }}>KrapMaps <span style={{ color:C.pink }}>HQ</span></div>
              <div style={{ color:C.dim, fontSize:D.phone?10:12, fontFamily:C.fontBody, marginTop:2 }}>Bk + Harley · {new Date().toLocaleDateString("en-GB",{day:"numeric",month:"short"})}</div>
            </div>
          ...
          {upcomingCal.length===0
            ...
            : upcomingCal.slice(0,3).map(c=>(
              <div key={c.id} style={{ marginBottom:6 }}>
                <div style={{ color:C.text, fontSize:D.phone?10:11, fontFamily:C.fontBody, fontWeight:600, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c.title}</div>
          ...
          {topIdeas.length===0
            ...
            : topIdeas.slice(0,3).map(i=>(
              <div key={i.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6, gap:4 }}>
                <div style={{ color:C.text, fontSize:D.phone?10:11, fontFamily:C.fontBody, fontWeight:500, flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{i.title}</div>
                {i.aiScore?.viralityScore!=null && <div style={{ color:i.aiScore.viralityScore>=80?C.green:i.aiScore.viralityScore>=60?C.yellow:C.pink, fontFamily:C.fontHead, fontSize:D.phone?14:16, flexShrink:0 }}>{i.aiScore.viralityScore}</div>}
              </div>
            ))
          }
        </div>
