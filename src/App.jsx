        ...
        <div key={k.id} style={{ marginBottom:14 }}>
          ...
          {editing===k.id ? (
            <div>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>saveKey(k.id)} style={{ flex:1, padding:"9px", borderRadius:9, border:"none", background:C.pink, color:"#fff", fontFamily:C.fontBody, fontWeight:700, cursor:"pointer", fontSize:13 }}>SAVE</button>
                <button onClick={()=>setEditing(null)} style={{ padding:"9px 16px", borderRadius:9, border:`1px solid ${C.border}`, background:"transparent", color:C.dim, fontFamily:C.fontBody, fontWeight:700, cursor:"pointer" }}>CANCEL</button>
              </div>
            </div>
          ) : (
...
async function callAI(prompt, maxTokens=2000) {
  ...
  const apiKey = storedCfg?.keys?.anthropic || ANTHROPIC_KEY;
