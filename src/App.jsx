...
const perfScore = v => {
  ...
  // Like ratio score (0-40 points): 20%+ ratio = 40pts, 10%= 20pts, 5%= 10pts
  const r = ratio(v);
  const ratioScore = Math.min(40, r * 2);
  // View score (0-40 points): 10K+ organic = 40pts, 5K=20pts, 1K=5pts
  const viewBase = v.promoted ? v.views * 0.1 : v.views; // discount promoted
  ...
  const commentScore = Math.min(20, commentRate * 100);
  return Math.round(ratioScore + viewScore + commentScore);
};
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div style={{ display:"flex", gap:4, overflowX:"auto", scrollbarWidth:"none", WebkitOverflowScrolling:"touch", paddingBottom:2 }}>
          {analyticsSubs.map(s=>(
            <button key={s.id} onClick={()=>setSub(s.id)} style={{ background:activeSub===s.id?C.pink:"transparent", border:`1px solid ${activeSub===s.id?C.pink:C.border}`, borderRadius:20, padding:D.phone?"5px 12px":"6px 16px", color:"#fff", fontFamily:C.fontBody, fontWeight:700, fontSize:D.phone?10:11, cursor:"pointer", whiteSpace:"nowrap", opacity:activeSub===s.id?1:0.5, textTransform:"uppercase", letterSpacing:"0.04em" }}>{s.label}</button>
          ))}
        </div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:D.phone?6:7 }}>
                  {hookStats.map((h,i)=>(
                    <div key={h.hook.replace("->","-\u200B>")} style={{ display:"flex", alignItems:"center", gap:12 }}>
                      <div style={{ color:i===0?C.yellow:"#fff", fontSize:D.phone?9:10, width:D.phone?80:140, fontFamily:C.fontBody, fontWeight:i===0?700:500, textTransform:"uppercase", letterSpacing:"0.02em", opacity:i===0?1:0.8, lineHeight:1.2 }}>{i===0&&<span style={{color:C.yellow,marginRight:4}}>{Icon.star}</span>}{h.hook}</div>
                  ...
                  {sortedVideos.map((v,i)=>{
                    ...
                    return (
                      <div key={v.id} style={{ display:"grid", gridTemplateColumns:D.phone?"2fr 45px 45px 40px 36px":"1fr 55px 65px 60px 55px 36px", padding:D.phone?"10px 10px":"12px 16px", borderBottom:`1px solid ${C.border}`, gap:D.phone?4:8, alignItems:"center", background:i===0?C.pink+"08":"transparent" }}>
                        <div>
                          <div style={{ color:C.text, fontSize:D.phone?11:12, fontFamily:C.fontBody, fontWeight:600, marginBottom:3, lineHeight:1.3, overflow:"hidden", display:"-webkit-box", WebkitLineClamp:D.phone?2:3, WebkitBoxOrient:"vertical" }}>{i===0&&<span style={{ color:C.yellow, marginRight:4 }}>{Icon.star}</span>}{v.title}{ (() => { const ps = perfScore(v); return ps !== null ? (
        <span style={{ background:ps>=70?C.green+"25":ps>=50?C.yellow+"25":C.pink+"25", color:ps>=70?C.green:ps>=50?C.yellow:C.pink, fontSize:8, marginLeft:6, padding:"1px 5px", borderRadius:4, fontFamily:C.fontBody, fontWeight:700, letterSpacing:"0.06em" }}>{ps} PERF</span>
      ) : v._updated ? (
        <span style={{ color:C.cyan, fontSize:8, marginLeft:6, letterSpacing:"0.06em", fontFamily:C.fontBody }}>UPDATED</span>
      ) : null; })() }</div>
                          <div style={{ display:"flex", gap:3, flexWrap:"wrap" }}><Tag color={C.pink} size={D.phone?8:9}>{v.type}</Tag><Tag color={C.cyan} size={D.phone?8:9}>{v.hook}</Tag>{v.promoted&&<Tag color={C.yellow} size={D.phone?8:9}>PAID</Tag>}{v.crossPost&&<Tag color={C.purple} size={D.phone?8:9}>TT+IG</Tag>}</div>
                        </div>
                        {!D.phone&&<div style={{ color:"#fff", fontSize:10, opacity:0.5, fontFamily:C.fontBody }}>{v.date?.slice(5)}</div>}
                        <div style={{ color:C.cyan, fontWeight:400, fontFamily:C.fontHead, fontSize:D.phone?13:14, letterSpacing:"0.03em" }}>{fmt(v.views)}</div>
                        ...
                        <div style={{ display:"flex", gap:4 }}>
                          <button onClick={()=>{ setUpdateTarget(v); openModal("updateVideo"); }} style={{ background:C.cyan+"20", border:`1px solid ${C.cyan}50`, borderRadius:6, color:C.cyan, cursor:"pointer", padding:"3px 8px", display:"flex", alignItems:"center", justifyContent:"center", gap:3, fontSize:9, fontFamily:C.fontBody, fontWeight:700 }}>{Icon.refresh}{!D.phone&&<span>24H</span>}</button>
