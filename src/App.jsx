...
function StatCard({ label, value, color, sub, onClick }) {
  return (
    <div onClick={onClick} style={{ background:`linear-gradient(135deg, ${C.card} 0%, ${color}08 100%)`, border:`1px solid ${color}30`, borderRadius:14, padding:"14px 16px", position:"relative", overflow:"hidden", cursor:onClick?"pointer":"default", boxShadow:`0 2px 12px ${color}12` }}>
      ...
      <div style={{ color, fontSize:D.fs.number, fontWeight:400, fontFamily:C.fontHead, lineHeight:1, textShadow:`0 0 16px ${color}40`, letterSpacing:"0.06em" }}>{value}</div>
...
// ─── AI ───────────────────────────────────────────────────────────────────────
const SYSTEM = `You are the growth strategist for KrapMaps. IMPORTANT: KrapMaps is a LITTER BIN FINDER app — it crowdsources the locations of public rubbish bins/trash cans on a map so people can find their nearest bin when they need to throw something away. It is NOT a toilet finder, NOT a bathroom app, NOT related to toilets in any way. It is gamified with leaderboards, badges, and rewards for mapping bins. Available on iOS and Android. TikTok @findkrap, also on Instagram. The core problem it solves: you are holding rubbish and cannot find a bin. KrapMaps shows you the nearest one. Best TikTok formula: Problem (holding rubbish, no bin in sight) → struggle → KrapMaps saves the day → satisfied reaction. Edgy/controversial hooks and facecam/street content outperform demos. Hook must land in 1-2 seconds. Instagram: polished, community-focused, aesthetic bin photography, before/after maps, UGC reposts. Mini-games: bin catcher slider + guess the country from bins quiz. Songkran April 13-15 2026 Thailand is a major activation — water festival means lots of outdoor activity and rubbish. Harley films in Thailand, Bk edits/strategises in UK. Respond ONLY with valid JSON — no markdown, no preamble.`;
...
function Dashboard({ keys, onEditKeys }) {
  ...
  useEffect(()=>{ saveJSON(VIDEOS_KEY,videos); if(sbLoaded) sbSet("km_videos",videos).catch(()=>{}); },[videos, sbLoaded]);
  ...
  useEffect(()=>{
    async function loadFromSupabase() {
      ...
      if(v) { try { const parsed=JSON.parse(v); if(Array.isArray(parsed)&&parsed.length>0) setVideos(parsed); } catch{} }
  ...
  async function scoreIdea(idea) {
    ...
    try {
      const r = await callAI(`You are scoring a TikTok content idea for KrapMaps (a litter bin finder app, NOT a toilet app). Be precise and varied with scores — avoid round numbers, each idea should score differently based on its genuine merit. A generic idea should score 40-55, a good idea 60-75, an excellent idea 76-90, exceptional ideas only get 90+. Return JSON only: {"viralityScore": precise integer 0-100 based on viral potential for TikTok,"hookScore": precise integer 0-100 based on hook strength in first 2 seconds,"verdict":"one punchy honest sentence — be critical if needed","viralityReason":"one sentence explaining the viral score","hookFeedback":"one sentence on hook strength","improvedHook":"rewrite the hook to be stronger","bestFormat":"facecam|street|screencap","recommendations":[{"action":"specific actionable improvement","impact":"high|medium|low"},{"action":"specific actionable improvement","impact":"high|medium|low"},{"action":"specific actionable improvement","impact":"high|medium|low"}]}. Idea:"${idea.title}" Type:${idea.type} Hook:${idea.hook} Notes:${idea.notes||"none"}`, 1000);
  ...
  const navBtnStyle = (id) => ({
    ...
    padding:D.phone?"6px 0 8px":"8px 0 10px", flex:1, background:"none", border:"none", cursor:"pointer",
  ...
  const HomeView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ borderRadius:D.phone?14:20, padding:D.phone?"16px 16px":"20px 22px", position:"relative", overflow:"hidden", background:"linear-gradient(135deg,#120020 0%,#1E0035 50%,#120020 100%)", border:`1px solid ${C.pink}30`, boxShadow:`0 8px 40px ${C.pink}20` }}>
        {/* Decorative orb */}
        ...
        <div style={{ position:"relative" }}>
          ...
          <div style={{ color:C.text, fontSize:D.phone?22:28, fontWeight:900, fontFamily:"'Sora', sans-serif", letterSpacing:"-0.02em", marginBottom:4 }}>KrapMaps <span style={{ color:C.pink, textShadow:`0 0 20px ${C.pink}` }}>HQ</span></div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:D.phone?"1fr":D.cols2, gap:D.gap }}>
          {[
            ...
            <button key={label} onClick={action} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 14px", cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:10, transition:"all 0.2s" }}>
              ...
              <div>
                <div style={{ color:C.text, fontSize:13, fontWeight:600, fontFamily:C.fontBody }}>{label}</div>
          ...
          {topIdeas.length===0
            ...
            : topIdeas.slice(0,3).map(i=>(
              <div key={i.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, gap:6 }}>
                ...
                {i.aiScore && <div style={{ color:i.aiScore.viralityScore>=80?C.green:i.aiScore.viralityScore>=60?C.yellow:C.pink, fontFamily:C.fontHead, fontWeight:400, fontSize:16, flexShrink:0 }}>{i.aiScore.viralityScore}</div>}
              </div>
            ))
          }
        </div>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {activeSub==="calendar" && (()=>{
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>Content Calendar</div>
            ...
            {filteredCal.length===0
              ...
              : filteredCal.map(c=>(
                <div key={c.id} style={{ background:C.card, borderLeft:`3px solid ${STATUS_C[c.status]||C.border}`, borderTop:`1px solid ${C.border}`, borderRight:`1px solid ${C.border}`, borderBottom:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px", display:"grid", gridTemplateColumns:"80px 1fr auto auto", gap:12, alignItems:"center" }}>
                  <div>
        ...
        {activeSub==="ideas" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>Idea Bank ({ideas.length})</div>
        ...
        {activeSub==="captions" && (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>Caption Generator</div>
            {(()=>{
              ...
              async function scanForCaption(file) {
                ...
                try {
                  ...
                  const resp=await fetch(scanUrl,{method:"POST",headers:scanHeaders,body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:300,system:SYSTEM,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:b64}},{type:"text",text:'What is this TikTok video about? Extract the main topic/concept in one sentence for caption generation. Return JSON only: {"title":"one sentence description of the video content","hook":"edgy/controversial|problem→solution|gamification|achievement|challenge|reaction|demo|other","notes":"any extra context"}'}]})});
                ...
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"16px" }}>
                  ...
                  <div style={{ display:"flex", gap:8, marginBottom:ideas.length>0?12:0, flexDirection:D.phone?"column":"row" }}>
                    <input
                      ...
                      style={{ flex:1, background:C.cardAlt, border:`1px solid ${C.border}`, borderRadius:10, color:C.text, padding:"10px 14px", fontSize:13, fontFamily:C.fontBody, outline:"none", boxSizing:"border-box" }}
                    />
            ...
            {captionResult && captionIdea && (
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                {/* TikTok caption */}
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px", borderTop:`1px solid ${C.pink}30` }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            {hookStats.length>0 && (
              <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                  {hookStats.map((h,i)=>(
                    <div key={h.hook} style={{ display:"flex", alignItems:"center", gap:12 }}>
                      <div style={{ color:i===0?C.yellow:"#fff", fontSize:10, width:D.phone?90:140, fontFamily:C.fontBody, fontWeight:i===0?700:500, textTransform:"uppercase", letterSpacing:"0.04em", opacity:i===0?1:0.8 }}>{i===0&&<span style={{color:C.yellow,marginRight:4}}>{Icon.star}</span>}{h.hook}</div>
            ...
            {videos.length===0
              ...
              : (
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, overflow:"hidden" }}>
                  <div style={{ display:"grid", gridTemplateColumns:D.phone?"2fr 45px 45px 40px 36px":"1fr 55px 65px 60px 55px 36px", padding:D.phone?"8px 10px":"10px 16px", background:C.pinkBg, color:"#fff", fontSize:D.phone?8:9, fontWeight:700, letterSpacing:"0.08em", gap:D.phone?4:8, fontFamily:C.fontBody, opacity:0.7 }}>
                    {(D.phone?["TITLE","VIEWS","LIKES","RATIO",""] : ["TITLE","DATE","VIEWS","LIKES","RATIO",""]).map(h=><div key={h}>{h}</div>)}
                  </div>
                  {sortedVideos.map((v,i)=>{
                    const r=ratio(v); const hot=r>=5;
                    return (
                      <div key={v.id} style={{ display:"grid", gridTemplateColumns:D.phone?"2fr 45px 45px 40px 36px":"1fr 55px 65px 60px 55px 36px", padding:D.phone?"10px 10px":"12px 16px", borderBottom:`1px solid ${C.border}`, gap:D.phone?4:8, alignItems:"center", background:i===0?C.pink+"08":"transparent" }}>
                        <div>
                          <div style={{ color:C.text, fontSize:12, fontFamily:C.fontBody, fontWeight:600, marginBottom:4, lineHeight:1.3 }}>{i===0&&<span style={{ color:C.yellow, marginRight:4 }}>{Icon.star}</span>}{v.title}{v._updated&&<span style={{ color:C.cyan, fontSize:8, marginLeft:6, letterSpacing:"0.06em", fontFamily:C.fontBody }}>UPDATED</span>}</div>
                        ...
                        {!D.phone&&<div style={{ color:"#fff", fontSize:10, opacity:0.5, fontFamily:C.fontBody }}>{v.date?.slice(5)}</div>}
                        <div style={{ color:C.cyan, fontWeight:400, fontFamily:C.fontHead, fontSize:14, letterSpacing:"0.03em" }}>{fmt(v.views)}</div>
                        <div style={{ color:C.pink, fontWeight:400, fontFamily:C.fontHead, fontSize:D.phone?12:14, letterSpacing:"0.03em" }}>{fmt(v.likes)}</div>
                        <div style={{ color:hot?C.yellow:"#fff", fontWeight:400, fontFamily:C.fontHead, fontSize:D.phone?12:14, opacity:hot?1:0.6 }}>{r.toFixed(1)}%</div>
                        <div style={{ display:"flex", gap:4 }}>
                          <button onClick={()=>{ setUpdateTarget(v); openModal("updateVideo"); }} style={{ background:C.cyan+"15", border:`1px solid ${C.cyan}40`, borderRadius:6, color:C.cyan, cursor:"pointer", padding:"3px 7px", display:"flex", alignItems:"center", justifyContent:"center" }} title="Update 24hr stats">{Icon.refresh}</button>
        ...
        {activeSub==="analysis" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>What's Working</div>
                ...
                <>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:D.cols2, gap:D.gap }}>
                    {[{label:"HOOK",data:analysis.hookAnalysis,bk:"bestHook",wk:"worstHook",color:C.cyan},{label:"FORMAT",data:analysis.typeAnalysis,bk:"bestType",wk:"worstType",color:C.green}].map(({label,data,bk,wk,color})=>(
                      <div key={label} style={{ background:C.card, border:`1px solid ${color}20`, borderRadius:14, padding:"16px" }}>
        ...
        {activeSub==="nextVids" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>Next 3 Videos</div>
        ...
        {activeSub==="weekly" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>Harley Brief</div>
        ...
        {activeSub==="trends" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>Trend Analysis</div>
  ...
  const TasksView = () => {
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        {/* Header */}
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>TASKS & IDEAS</div>
              ...
              {aiRecs.recommendations?.map((r,i)=>(
                <div key={i} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10 }}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ color:C.textMed, fontSize:12, lineHeight:1.6 }}>{r.why}</div>
                  </div>
        ...
        {activeTab==="ideas" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {/* Add idea */}
            <div style={{ display:"flex", gap:8 }}>
              <input value={ideaInput} onChange={e=>setIdeaInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addIdea()} placeholder="Add an idea..." style={{ flex:1, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, color:C.text, padding:"10px 14px", fontSize:13, fontFamily:C.fontBody, outline:"none", boxSizing:"border-box" }} />
            ...
            {appIdeas.length===0
              ...
              : appIdeas.map(idea=>(
                <div key={idea.id} style={{ background:C.card, border:`1px solid ${idea.score?.appScore>=70?C.cyan+"40":C.border}`, borderRadius:14, padding:"14px 16px" }}>
                  {/* Idea header */}
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10, marginBottom:idea.score?10:0 }}>
                    <div style={{ flex:1 }}>
                      <div style={{ color:C.text, fontSize:13, fontFamily:C.fontBody, fontWeight:600, lineHeight:1.3, lineHeight:1.4, textTransform:"none" }}>{idea.text}</div>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    ...
                    <Btn small outline color={C.pink} onClick={()=>{ setTasks(ts=>[{ id:Date.now(), text:idea.text, done:false, assignee:"Both", created:today() },...ts]); setActiveTab("todo"); }}>→ TODO</Btn>
  ...
  const GrowthView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>Growth Stats</div>
        <Btn small outline color={C.pink} onClick={()=>openModal("editStats")}>Update Stats</Btn>
      </div>

      {/* TikTok */}
      <div style={{ background:C.card, border:`1px solid ${C.borderStrong}`, borderRadius:16, padding:"18px 20px", borderTop:`1px solid ${C.pink}30` }}>
  ...
  const SettingsView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ color:C.text, fontSize:D.fs.title, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.06em" }}>Settings</div>
        ...
        const resp = await fetch(scanUrl, {
          ...
          body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:400, system:SYSTEM,
            messages:[{ role:"user", content:[
              ...
              { type:"text", text:'TikTok analytics screenshot. Extract ONLY the numbers. Return JSON only: {"views":number or null,"likes":number or null,"comments":number or null,"shares":number or null,"confidence":"high|medium|low"}. Convert K to thousands, M to millions. Null if not visible.' }
            ]}]
          })
        });
      ...
      <Modal title="Update Stats" onClose={()=>closeModal("updateVideo")}>
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ background:C.cardAlt, borderRadius:10, padding:"10px 14px" }}>
            ...
            <div style={{ color:C.text, fontSize:13, fontWeight:600, fontFamily:C.fontBody, lineHeight:1.3 }}>{v.title}</div>
