...
const KEYS_KEY    = "krapmaps_v44_config";
const MANUAL_KEY  = "krapmaps_v44_manual";
const VIDEOS_KEY  = "krapmaps_v44_videos";
const IDEAS_KEY   = "krapmaps_v44_ideas";
const CAL_KEY     = "krapmaps_v44_calendar";
const TRENDS_KEY  = "krapmaps_v44_trends";
const TASKS_KEY   = "krapmaps_v44_tasks";
const APPIDEAS_KEY = "krapmaps_v44_appideas";

const SYNC_KEY   = "krapmaps_v44_syncurl";
...
function Dashboard({ keys, onEditKeys }) {
                    ...
                    {idea.aiScore && (
                      <div style={{ display:"flex", gap:10, alignItems:"center", flexShrink:0 }}>
                        <div style={{ textAlign:"center" }}>
                          <div style={{ color:idea.aiScore.appScore>=80?C.green:idea.aiScore.appScore>=60?C.yellow:C.pink, fontSize:28, fontWeight:400, fontFamily:C.fontHead, lineHeight:1 }}>{idea.aiScore.appScore??idea.aiScore.viralityScore}</div>
                          <div style={{ color:C.dim, fontSize:8, fontFamily:C.fontBody, fontWeight:700, letterSpacing:"0.1em" }}>APP</div>
                        </div>
                        <div style={{ textAlign:"center" }}>
                          <div style={{ color:C.cyan, fontSize:20, fontWeight:400, fontFamily:C.fontHead, lineHeight:1 }}>{idea.aiScore.viralityScore}</div>
                          <div style={{ color:C.dim, fontSize:8, fontFamily:C.fontBody, fontWeight:700, letterSpacing:"0.1em" }}>VIRAL</div>
                        </div>
                        <div style={{ textAlign:"center" }}>
                          <div style={{ color:C.purple, fontSize:20, fontWeight:400, fontFamily:C.fontHead, lineHeight:1 }}>{idea.aiScore.hookScore}</div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              : pendingTasks.map(t=>(
                <div key={t.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px", display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ color:C.dim, fontSize:10, marginTop:2 }}>{new Date(t.created).toLocaleDateString("en-GB",{day:"numeric",month:"short"})}</div>
                  </div>
        ...
        {activeTab==="ideas" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {/* Add idea */}
            <div style={{ display:"flex", gap:8 }}>
              <input value={ideaInput} onChange={e=>setIdeaInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addIdea()} placeholder="Add a feature idea..." style={{ flex:1, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, color:C.text, padding:"10px 14px", fontSize:13, fontFamily:C.fontBody, outline:"none", boxSizing:"border-box" }} />
              ...
              : appIdeas.map(idea=>(
                <div key={idea.id} style={{ background:C.card, border:`1px solid ${C.cyan}20`, borderRadius:12, padding:"12px 14px", display:"flex", alignItems:"flex-start", gap:12 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ color:C.dim, fontSize:10, marginTop:3 }}>{new Date(idea.created).toLocaleDateString("en-GB",{day:"numeric",month:"short"})}</div>
                  </div>
                  <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                    <button onClick={()=>{ setTasks(ts=>[{ id:Date.now(), text:idea.text, done:false, assignee:"Both", created:today() },...ts]); setActiveTab("todo"); }} style={{ background:C.pink+"20", border:`1px solid ${C.pink}40`, borderRadius:7, color:"#fff", cursor:"pointer", padding:"5px 12px", fontSize:10, fontFamily:C.fontBody, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em" }}>→ TODO</button>
  ...
  const AddIdeaModal = () => {
    ...
    function saveAndScore() {
      ...
      addIdea(newIdea, false);
      // Score in background after modal closes using the top-level scoreIdea
      setTimeout(() => scoreIdea(newIdea), 300);
    }
