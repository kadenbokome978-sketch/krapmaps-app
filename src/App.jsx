      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:D.phone?8:D.gap }}>
          {[
            ...
            <button key={label} onClick={action} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:D.phone?"12px 10px":"14px 14px", cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:D.phone?8:10, transition:"all 0.2s" }}>
              <div style={{ width:D.phone?28:36, height:D.phone?28:36, borderRadius:8, background:C.pink+"18", border:`1px solid ${C.pink}25`, display:"flex", alignItems:"center", justifyContent:"center", color:C.pink, flexShrink:0 }}>{icon}</div>
              <div style={{ minWidth:0 }}>
                <div style={{ color:C.text, fontSize:D.phone?11:13, fontWeight:600, fontFamily:C.fontBody, lineHeight:1.2 }}>{label}</div>
                {!D.phone && <div style={{ color:C.dim, fontSize:9, marginTop:2 }}>{sub}</div>}
              </div>
            </button>
          ))}
        </div>
