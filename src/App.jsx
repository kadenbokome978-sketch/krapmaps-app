...
function Dashboard({ keys, onEditKeys }) {
  ...
  const TasksView = () => {
    ...
    async function scoreAppIdea(idea) {
      ...
      try {
        const r = await callAI('Score this KrapMaps app feature idea. Return JSON only: {"appScore":0-100 (precise integer e.g. 73),"verdict":"one punchy sentence","impact":"high|medium|low","effort":"high|medium|low","recommendation":"one specific action to implement or improve this idea"}. Idea:"' + idea.text + '"', 600);
            ...
            {pendingTasks.length===0
              ...
              : pendingTasks.map(t=>(
                <div key={t.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px", display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ color:C.text, fontSize:13, fontFamily:C.fontBody, fontWeight:500, textTransform:"none" }}>{t.text}</div>
