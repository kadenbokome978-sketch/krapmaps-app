                ...
                {expanded===idea.id && (
                  <>
                    ...
                    {idea.altHooks?.length>0 && (
                      <div style={{ background:C.cardAlt, borderRadius:9, padding:"9px 10px", marginBottom:10 }}>
                        <SLabel color={C.purple} mb={8}>3 ALTERNATIVE HOOKS</SLabel>
                        {idea.altHooks.map((h,hi)=>(
                          <div key={hi} style={{ padding:"8px 10px", borderRadius:8, background:`${C.purple}08`, border:`1px solid ${C.purple}20`, marginBottom:hi<2?6:0 }}>
                            <div style={{ fontSize:11, fontWeight:700, color:C.text, marginBottom:3 }}>"{h.hook}"</div>
                            <div style={{ display:"flex", gap:5 }}><Tag color={C.purple}>{h.type}</Tag><div style={{ fontSize:9, color:C.dim }}>{h.why}</div></div>
                          </div>
                        ))}
                      </div>
                    )}
      ...
      {sub==="TO DO" && (
        <>
          ...
          <div style={{ display:"flex", gap:6, marginBottom:4 }}>
            {["ALL","BK","HARLEY","BOTH"].map(a=><Pill key={a} active={taskFilter===a} color={C.yellow} onClick={()=>setTaskFilter(a)}>{a}</Pill>)}
          </div>
          <div style={{ display:"flex", gap:6 }}>
          ...
          {appIdeas.map(idea=>(
            <Glass key={idea.id} glow={idea.score>=70?C.cyan:undefined} style={{ padding:"14px" }}>
              ...
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                ...
                <ActionBtn color={C.yellow} onClick={()=>{ setEditAppIdeaTarget&&setEditAppIdeaTarget(idea); setModals&&setModals(m=>({...m,editAppIdea:true})); }}>EDIT</ActionBtn>
                <ActionBtn color={C.cyan} onClick={()=>{ setTasks(ts=>[{id:Date.now(),text:idea.text,assignee:"BK",done:false},...ts]); }}>TO DO</ActionBtn>
...
const SettingsView = ({ keys, onEditKeys, scrapedStats, hasIG, WL }) => {
  ...
  return (<div style={{ display:"flex", flexDirection:"column", gap:10 }}>
    ...
    {/* API KEYS - editable */}
    <Glass style={{ padding:"14px", border:`1px solid ${C.pink}25` }}>
      <SLabel color={C.pink} mb={4}>API KEYS</SLabel>
      <div style={{ fontSize:10, color:C.dim, marginBottom:12, lineHeight:1.5 }}>Get your Anthropic key from console.anthropic.com</div>
      {[
        { id:"anthropic", label:"ANTHROPIC API", desc:"AI scoring + captions (console.anthropic.com)" },
        { id:"ig", label:"INSTAGRAM TOKEN", desc:"IG live stats (developers.facebook.com)" },
      ].map((k)=>(
        <div key={k.id} style={{ marginBottom:14 }}>
          <Row style={{ marginBottom:6 }}>
            <div>
              <div style={{ fontSize:12, fontWeight:700, color:C.text }}>{k.label}</div>
              <div style={{ fontSize:9, color:C.dim, marginTop:2 }}>{k.desc}</div>
            </div>
            <Tag color={keys?.[k.id]?C.green:C.pink}>{keys?.[k.id]?"SET":"MISSING"}</Tag>
          </Row>
          {editing===k.id ? (
            <div>
              <input value={draftKey} onChange={e=>setDraftKey(e.target.value)} placeholder={`Paste ${k.label} here...`} style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:`1px solid ${C.pink}40`, borderRadius:10, color:C.text, padding:"9px 12px", fontSize:14, fontFamily:C.fontMono, outline:"none", boxSizing:"border-box", marginBottom:8 }} />
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>saveKey(k.id)} style={{ flex:1, padding:"11px", borderRadius:9, border:"none", background:C.pink, color:"#fff", fontFamily:C.fontBody, fontWeight:700, cursor:"pointer", fontSize:14 }}>SAVE KEY</button>
                <button onClick={()=>setEditing(null)} style={{ padding:"11px 16px", borderRadius:9, border:`1px solid ${C.border}`, background:"transparent", color:C.dim, fontFamily:C.fontBody, fontWeight:700, cursor:"pointer" }}>CANCEL</button>
              </div>
            </div>
          ) : (
            <button onClick={()=>{ setEditing(k.id); setDraftKey(keys?.[k.id]||""); }} style={{ width:"100%", padding:"8px", borderRadius:9, border:`1px solid ${keys?.[k.id]?C.border:C.pink+"40"}`, background:keys?.[k.id]?"rgba(255,255,255,0.03)":`${C.pink}08`, color:keys?.[k.id]?C.dim:C.pink, fontFamily:C.fontBody, fontSize:11, fontWeight:700, cursor:"pointer", letterSpacing:"0.06em" }}>
              {keys?.[k.id]?"CHANGE KEY":"+ ADD KEY"}
            </button>
          )}
        </div>
      ))}
    ...
    {/* Status */}
    <Glass style={{ padding:"14px" }}>
      <SLabel mb={12}>SYSTEM STATUS</SLabel>
      {[
        {label:"Supabase DB",value:"CONNECTED",color:C.green},
        {label:"Apify scraper",value:scrapedStats?"LAST RAN "+(()=>{try{const h=Math.round((Date.now()-new Date(scrapedStats.scraped_at))/3600000);return h<1?"<1h ago":h+"h ago";}catch{return "unknown";}})():"NOT YET RUN",color:scrapedStats?C.green:C.yellow},
        {label:"Instagram",value:hasIG?"CONNECTED":"ADD KEY ABOVE",color:hasIG?C.green:C.yellow},
        {label:"AI (Anthropic)",value:keys?.anthropic?"KEY SET":"ADD KEY ABOVE",color:keys?.anthropic?C.green:C.pink},
      ].map((r,i,arr)=>(
...
function Dashboard({ keys, onEditKeys }) {
  ...
  const EditIdeaModal = () => {
    ...
    const saveAndRescore = () => {
      ...
      const updated = {...idea,...form, viral:0, hookScore:0, verdict:"Edited -- tap SCORE to re-analyse", viralReason:null, hookFeedback:null, improvedHook:null, recs:null, altHooks:altHooks||idea.altHooks||null };
      setIdeas(is=>is.map(i=>i.id===idea.id?updated:i));
      closeModal("editIdea");
    };

    const getAltHooks = async () => {
      if(loadingHooks) return;
      setLoadingHooks(true);
      try {
        const r = await callAI(`Give 3 alternative hook openings for this TikTok idea: "${form.title}". Return JSON: {"hooks":[{"hook":"string","type":"pov|reaction|challenge|achievement","why":"one sentence"}]}`, 600);
        const hooks = r.hooks||[];
        setAltHooks(hooks);
        setIdeas(is=>is.map(i=>i.id===idea.id?{...i,altHooks:hooks}:i));
      } catch(e) { setAiErr("Hooks failed: "+e.message); }
    ...
    return (
      <ModalBase onClose={()=>closeModal("editIdea")}>
        ...
        <div style={{ fontSize:10,color:C.dim,marginBottom:16 }}>Edit then save -- scores reset so tap SCORE again after</div>

        <MLabel>Idea Description</MLabel>
        <textarea value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="Describe the video idea..." rows={3} style={{ width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${C.pink}40`,borderRadius:10,color:C.text,padding:"10px 12px",fontSize:16,fontFamily:C.fontBody,outline:"none",boxSizing:"border-box",resize:"none",marginBottom:12,lineHeight:1.5 }} />

        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12 }}>
          <div>
            <MLabel>TYPE</MLabel>
            <select value={form.type} onChange={set("type")} style={{ width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,color:C.text,padding:"10px 12px",fontSize:15,fontFamily:C.fontBody,outline:"none",boxSizing:"border-box" }}>
              {VIDEO_TYPES.map(t=><option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <MLabel>HOOK TYPE</MLabel>
            <select value={form.hook} onChange={set("hook")} style={{ width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,color:C.text,padding:"10px 12px",fontSize:15,fontFamily:C.fontBody,outline:"none",boxSizing:"border-box" }}>
              {HOOK_TYPES.map(t=><option key={t} value={t}>{t}</option>)}
            </select>
          </div>
        </div>
        ...
        <div style={{ marginBottom:14 }}>
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8 }}>
            <MLabel style={{ marginBottom:0 }}>3 ALTERNATIVE HOOKS</MLabel>
            <button onClick={getAltHooks} disabled={loadingHooks} style={{ padding:"4px 12px",borderRadius:8,border:`1px solid ${C.purple}40`,background:`${C.purple}18`,color:C.purple,fontFamily:C.fontBody,fontSize:10,fontWeight:700,cursor:"pointer",opacity:loadingHooks?0.5:1 }}>
              {loadingHooks?"GENERATING...":"GENERATE"}
            </button>
          </div>
          {(altHooks||idea.altHooks) ? (altHooks||idea.altHooks).map((h,i)=>(
            <div key={i} onClick={()=>setForm(f=>({...f,title:h.hook}))} style={{ padding:"10px 12px",borderRadius:10,background:`${C.purple}08`,border:`1px solid ${C.purple}20`,marginBottom:i<2?8:0,cursor:"pointer" }}>
              <div style={{ fontSize:11,fontWeight:700,color:C.text,marginBottom:3,lineHeight:1.4 }}>"{h.hook}"</div>
              <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                <Tag color={C.purple}>{h.type}</Tag>
                <div style={{ fontSize:9,color:C.dim }}>{h.why}</div>
              </div>
            </div>
          )) : (
            <div style={{ padding:"12px",borderRadius:10,background:"rgba(255,255,255,0.03)",border:`1px solid ${C.border}`,textAlign:"center" }}>
              <div style={{ fontSize:10,color:C.dim }}>Tap GENERATE for 3 AI hook ideas</div>
            </div>
          )}
        </div>

        <MBtn onClick={saveAndRescore}>SAVE & RESET SCORES</MBtn>
      </ModalBase>
    );
  };
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", fontFamily:C.fontBody, position:"relative" }}>
      ...
      <div style={{ maxWidth:390, margin:"0 auto", padding:"16px 14px 92px", position:"relative", zIndex:1 }}>
        ...
        {/* NO KEY BANNER */}
        {!keys?.anthropic && (
          <div onClick={()=>setNav("settings")} style={{ margin:"0 0 10px 0", padding:"12px 16px", borderRadius:14, background:`${C.pink}18`, border:`1px solid ${C.pink}40`, cursor:"pointer", display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ fontSize:20 }}>⚠️</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:12, fontWeight:700, color:C.pink }}>ADD ANTHROPIC API KEY</div>
              <div style={{ fontSize:10, color:C.textMed, marginTop:2 }}>AI features won't work until you add your key. Tap here → Settings.</div>
            </div>
            <div style={{ fontSize:16, color:C.pink }}>›</div>
          </div>
        )}
        ...
        {/* AI ERROR */}
        {aiErr && (
          <div style={{ position:"fixed", top:54, left:14, right:14, background:"rgba(10,5,20,0.98)", border:`1px solid ${C.pink}60`, borderRadius:14, padding:"12px 16px", color:"#FF8888", fontSize:12, zIndex:999, display:"flex", gap:10, alignItems:"flex-start", backdropFilter:"blur(20px)", boxShadow:`0 8px 32px rgba(0,0,0,0.6)` }}>
            ...
            <button onClick={()=>setAiErr(null)} style={{ background:"none",border:"none",color:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:18,lineHeight:1,flexShrink:0 }}>×</button>
          </div>
        )}
      </div>
