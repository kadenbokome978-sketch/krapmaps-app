...
function Dashboard({ keys, onEditKeys }) {
  ...
  async function scoreIdea(idea) {
    ...
    try {
(() => {
      const scorePrompt = `You are scoring a TikTok content idea for KrapMaps -- a gamified bin-finding app. NOT a toilet app. Users find bins, map them, earn points.

CONTEXT: @findkrap posts street content, facecam reactions, achievement reveals, gamification clips. Best performers: "Tired of holding rubbish?" (23% like ratio), "Rating bins 10/10 FROM THE APP" (20% ratio), "POV you find the best bin" (13% ratio). Audience: young, fun, global.

SCORING RULES:
- Score based on CONCEPT STRENGTH and EXECUTION POTENTIAL, not whether it contains specific words
- A creative or unusual idea can score high even without obvious viral hooks
- Score viralityScore and hookScore INDEPENDENTLY -- they measure different things
- viralityScore = will people share, comment, follow after watching? (concept level)
- hookScore = does the OPENING LINE grab attention in 2 seconds? (execution level)
- Be varied: same score twice in a row is wrong. Base it on genuine merit
- High scores (75+) require genuine mass appeal or strong emotional reaction potential
- Medium scores (50-70) for solid ideas that need refinement
- Low scores (30-49) for generic or unclear concepts only

Idea title: "${idea.title}"
Video type: ${idea.type}
Hook/opening: ${idea.hook}
Creator notes: ${idea.notes||"none -- score based on title alone"}

Return JSON only, no markdown, no explanation outside JSON:
{
  "viralityScore": integer 0-100,
  "hookScore": integer 0-100,
  "verdict": "honest 1 sentence on the overall concept",
  "viralityReason": "1 sentence on why this will or wont spread, specific to KrapMaps audience",
  "hookFeedback": "1 sentence on the opening line strength only",
  "improvedHook": "rewrite the opening to be stronger and more specific",
  "bestFormat": "facecam|street|screencap",
  "recommendations": [
    {"action": "one specific change to improve this idea", "impact": "high|medium|low"},
    {"action": "one specific change to improve this idea", "impact": "high|medium|low"},
    {"action": "one specific change to improve this idea", "impact": "high|medium|low"}
  ]
}`;
  ...
  const AddIdeaModal = () => {
    ...
    function saveAndScore() {
      ...
      // Add to state first, then close modal, then score
      setIdeas(is=>[newIdea,...is]);
      closeModal("addIdea");
      // Score after state has updated
      setTimeout(() => scoreIdea(newIdea), 500);
    }
