...
function Dashboard({ keys, onEditKeys }) {
  ...
  useEffect(()=>{
    if(!sbLoaded) return; // Wait until Supabase has loaded before saving back
    ...
    // Only write to Supabase if user has made manual changes (not just loaded from scraper)
    if(sbLoaded) sbSet("km_videos",videos).catch(()=>{});
  },[videos, sbLoaded]);
  ...
  // Load scraped stats from Supabase with retry
  const [scrapedStats, setScrapedStats] = useState(null);
  const [statsLoading, setStatsLoading] = useState(true);
  const [statsError, setStatsError] = useState(null);
  useEffect(()=>{
    async function loadScraped(attempt=0) {
      ...
      try {
        ...
        if(Array.isArray(data) && data.length > 0) {
          ...
          // Always reload km_videos after scraper runs to pick up Apify additions
          try {
            const vRes = await fetch(getSbUrl() + "/rest/v1/km_videos?order=updated_at.desc&limit=1", {
              headers: { "apikey": getSbKey(), "Authorization": "Bearer " + getSbKey() }
            });
            const vData = await vRes.json();
            if(Array.isArray(vData) && vData.length > 0) {
              const vParsed = JSON.parse(vData[0].value);
              if(Array.isArray(vParsed) && vParsed.length > 0) {
                const seen=new Set();
                const deduped=vParsed.filter(v=>{ const id=String(v.id||''); if(seen.has(id)) return false; seen.add(id); return true; });
                // Only update if scraper has more videos than we currently have
                if(deduped.length >= videos.length) setVideos(deduped);
              }
            }
          } catch{}
        } else {
          setStatsError("No data yet -- run the scraper");
        }
      } catch(e) {
        if(attempt < 2) {
          setTimeout(()=>loadScraped(attempt+1), 3000);
        } else {
          setStatsError("Load failed -- tap to retry");
        }
      }
  ...
  const updateIdea  = (id, updates) => setIdeas(is=>is.map(i=>i.id===id?{...i,...updates}:i));
        ...
        <div style={{ position:"relative" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              ...
              {statsError && <div style={{ color:"#FF6B6B", fontSize:8, fontFamily:C.fontBody, marginTop:3, opacity:0.7, cursor:"pointer" }} onClick={()=>window.location.reload()}>{statsError} (tap to refresh)</div>}
            </div>
              ...
              : topIdeas.map(idea=>(
                <div key={idea.id} style={{ background:C.card, border:`1px solid ${(idea.aiScore?.viralityScore>=7||idea.aiScore?.viralityScore>=70)?C.pink+"44":C.border}`, borderRadius:14, padding:D.phone?"12px 14px":"16px 18px" }}>
                  ...
                  <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
                    ...
                    <Btn small outline color={C.yellow} onClick={()=>{ setEditIdeaTarget(idea); setModals(m=>({...m,editIdea:true})); }}>EDIT</Btn>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <div style={{ display:"flex", gap:6 }}>
                {videos.length>0 && <Btn small outline color="#FF5555" onClick={()=>{ if(window.confirm && window.confirm("Remove all "+videos.length+" videos? This cannot be undone.")) { setVideos([]); } else if(!window.confirm) { setVideos([]); } }}>Clear All</Btn>}
              ...
              : appIdeas.map(idea=>(
                <div key={idea.id} style={{ background:C.card, border:`1px solid ${idea.score?.appScore>=70?C.cyan+"40":C.border}`, borderRadius:14, padding:"14px 16px" }}>
                  ...
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    ...
                    <Btn small outline color={C.yellow} onClick={()=>{ setEditAppIdeaTarget(idea); setModals(m=>({...m,editAppIdea:true})); }}>EDIT</Btn>
  ...
  function Empty({ icon, title, sub, action, actionLabel, disabled=false, color=C.pink }) {
    return (
      <div style={{ background:C.card, border:`1px dashed ${C.border}40`, borderRadius:16, padding:"40px 20px", textAlign:"center" }}>
  ...
  // EDIT IDEA MODAL (content ideas)
  const EditIdeaModal = () => {
    const idea = editIdeaTarget;
    const [form, setForm] = useState({ title:idea?.title||"", type:idea?.type||"facecam", hook:idea?.hook||"other", notes:idea?.notes||"" });
    const set = (k,v) => setForm(f=>({...f,[k]:v}));
    if(!idea) return null;
    function save() {
      if(!form.title?.trim()) return;
      updateIdea(idea.id, { title:form.title.trim(), type:form.type, hook:form.hook, notes:form.notes });
      setModals(m=>({...m,editIdea:false}));
      setEditIdeaTarget(null);
    }
    return (
      <Modal title="Edit Idea" onClose={()=>{ setModals(m=>({...m,editIdea:false})); setEditIdeaTarget(null); }}>
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          <FInput label="IDEA / HOOK TITLE *" value={form.title} onChange={e=>set("title",e.target.value)} />
          ...
          <FInput label="NOTES" value={form.notes} onChange={e=>set("notes",e.target.value)} />
          <div style={{ display:"flex", gap:8 }}>
            <Btn full color={C.yellow} onClick={save}>SAVE CHANGES</Btn>
            <Btn full outline color={C.purple} onClick={()=>{ save(); setTimeout(()=>scoreIdea({...idea,...form,title:form.title.trim()}),300); }}>SAVE & RE-SCORE</Btn>
          </div>
        </div>
      </Modal>
    );
  ...
  // EDIT APP IDEA MODAL (tasks tab ideas)
  const EditAppIdeaModal = () => {
    ...
    function save() {
      if(!text.trim()) return;
      setAppIdeas(is=>is.map(i=>i.id===idea.id?{...i,text:text.trim()}:i));
      setModals(m=>({...m,editAppIdea:false}));
      setEditAppIdeaTarget(null);
    }
    return (
      <Modal title="Edit Idea" onClose={()=>{ setModals(m=>({...m,editAppIdea:false})); setEditAppIdeaTarget(null); }}>
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          <FInput label="IDEA" value={text} onChange={e=>setText(e.target.value)} />
          <div style={{ display:"flex", gap:8 }}>
            <Btn full color={C.yellow} onClick={save}>SAVE</Btn>
            <Btn full outline color={C.purple} onClick={()=>{ save(); const updated={...idea,text:text.trim()}; setTimeout(()=>{ setAppIdeas(is=>is.map(i=>i.id===idea.id?updated:i)); },100); }}>SAVE & RE-SCORE</Btn>
          </div>
        </div>
      </Modal>
    );
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0, background:"#0A0012EE", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", borderTop:`1px solid ${C.borderStrong}`, display:"flex", zIndex:50, boxShadow:"0 -8px 32px rgba(0,0,0,0.5)" }}>
        {NAV.map(n=>(
          <button key={n.id} onClick={()=>{ setNav(n.id); setSub(null); }} style={navBtnStyle(n.id)}>
            <div style={{ width:34, height:34, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", background:nav===n.id?C.pink+"20":"transparent", border:nav===n.id?`1px solid ${C.pink}60`:"1px solid transparent", transition:"all 0.2s" }}>{n.icon}</div>
            <span>{n.label}</span>
          </button>
        ))}
      </div>
