...
function Modal({ title, onClose, children, maxWidth=500 }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div style={{ background:C.card, border:`1px solid ${C.borderStrong}`, borderRadius:D.phone?"18px 18px 0 0":18, padding:D.phone?18:22, width:"100%", maxWidth:D.phone?"100%":maxWidth, maxHeight:D.phone?"85vh":"90vh", overflowY:"auto", boxShadow:`0 20px 60px rgba(0,0,0,0.8), 0 0 0 1px ${C.borderStrong}` }} onClick={e=>e.stopPropagation()}>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
      <svg width={size} height={size}>
        ...
        <text x={size/2} y={size/2+4} textAnchor="middle" fill={color} fontSize="11" fontWeight="800" fontFamily="Bebas Neue, sans-serif">{safeScore}</text>
      </svg>
...
function Dashboard({ keys, onEditKeys }) {
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ borderRadius:D.phone?14:20, padding:D.phone?"16px 16px":"20px 22px", position:"relative", overflow:"hidden", background:"linear-gradient(135deg,#120020 0%,#1E0035 50%,#120020 100%)", border:`1px solid ${C.pink}30`, boxShadow:`0 8px 40px ${C.pink}20` }}>
        ...
        <div style={{ position:"relative" }}>
          ...
          <div style={{ marginTop:D.phone?10:16, display:"flex", gap:8, alignItems:"center" }}>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:D.phone?8:D.gap }}>
          {[
            ...
            <button key={label} onClick={action} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:D.phone?"10px 10px":"14px 14px", cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:D.phone?8:10, transition:"all 0.2s" }}>
              <div style={{ width:D.phone?30:36, height:D.phone?30:36, borderRadius:8, background:C.pink+"18", border:`1px solid ${C.pink}25`, display:"flex", alignItems:"center", justifyContent:"center", color:C.pink, flexShrink:0 }}>{icon}</div>
              <div>
                <div style={{ color:C.text, fontSize:D.phone?11:13, fontWeight:600, fontFamily:C.fontBody }}>{label}</div>
                <div style={{ color:C.dim, fontSize:D.phone?8:9, marginTop:2 }}>{sub}</div>
              </div>
            </button>
          ))}
        </div>
          ...
          {topIdeas.length===0
            ...
            : topIdeas.slice(0,3).map(i=>(
              <div key={i.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, gap:6 }}>
                ...
                {i.aiScore?.viralityScore!=null && <div style={{ color:i.aiScore.viralityScore>=80?C.green:i.aiScore.viralityScore>=60?C.yellow:C.pink, fontFamily:C.fontHead, fontWeight:400, fontSize:16, flexShrink:0 }}>{i.aiScore.viralityScore}</div>}
              </div>
            ))
          }
        </div>
      ...
      {videos.length>=2 && (
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
          ...
          <div style={{ display:"flex", gap:D.phone?6:8, flexWrap:"wrap" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {activeSub==="captions" && (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {captionResult?.tiktok && captionIdea && (
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                {/* TikTok caption */}
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:D.phone?"14px 16px":"18px 20px", borderTop:`1px solid ${C.pink}30` }}>
                  ...
                  {sortedVideos.map((v,i)=>{
                    ...
                    return (
                      <div key={v.id} style={{ display:"grid", gridTemplateColumns:D.phone?"2fr 45px 45px 40px 36px":"1fr 55px 65px 60px 55px 36px", padding:D.phone?"10px 10px":"12px 16px", borderBottom:`1px solid ${C.border}`, gap:D.phone?4:8, alignItems:"center", background:i===0?C.pink+"08":"transparent" }}>
                        <div>
                          <div style={{ color:C.text, fontSize:D.phone?11:12, fontFamily:C.fontBody, fontWeight:600, marginBottom:3, lineHeight:1.3, overflow:"hidden" }}>{i===0&&<span style={{ color:C.yellow, marginRight:4 }}>{Icon.star}</span>}{v.title}{ (() => { const ps = perfScore(v); return ps !== null ? (
        <span style={{ background:ps.score>=65?C.green+"25":ps.score>=50?C.yellow+"25":C.pink+"25", color:ps.score>=65?C.green:ps.score>=50?C.yellow:C.pink, fontSize:8, marginLeft:5, padding:"1px 6px", borderRadius:4, fontFamily:C.fontBody, fontWeight:700, letterSpacing:"0.06em" }}>{ps.label}</span>
      ) : null; })() }</div>
                      ...
                      {/* 24hr Performance Card */}
                      {(()=>{ const ps = perfScore(v); if(!ps) return null;
                        const r = ratio(v);
                        return (
                          <div style={{ gridColumn:"1/-1", background:`linear-gradient(135deg,${ps.score>=65?C.green:ps.score>=50?C.yellow:C.pink}08,transparent)`, border:`1px solid ${ps.score>=65?C.green:ps.score>=50?C.yellow:C.pink}25`, borderRadius:10, padding:"10px 14px", marginTop:2 }}>
                            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                              ...
                              <div style={{ display:"flex", alignItems:"baseline", gap:4 }}>
                                <div style={{ color:ps.score>=65?C.green:ps.score>=50?C.yellow:C.pink, fontSize:32, fontWeight:400, fontFamily:C.fontHead, lineHeight:1 }}>{ps.score}</div>
                                <div style={{ color:C.dim, fontSize:9, fontFamily:C.fontBody }}>/100</div>
                              </div>
                            </div>
                            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8 }}>
                              <div style={{ background:C.card, borderRadius:8, padding:"8px 10px", textAlign:"center" }}>
                                <div style={{ color:C.cyan, fontSize:D.phone?14:16, fontWeight:400, fontFamily:C.fontHead }}>{fmt(v.views)}</div>
                                <div style={{ color:C.dim, fontSize:8, letterSpacing:"0.08em", fontFamily:C.fontBody, fontWeight:700, marginTop:2 }}>VIEWS</div>
                              </div>
                              <div style={{ background:C.card, borderRadius:8, padding:"8px 10px", textAlign:"center" }}>
                                <div style={{ color:r>=10?C.green:r>=5?C.yellow:C.pink, fontSize:D.phone?14:16, fontWeight:400, fontFamily:C.fontHead }}>{r.toFixed(1)}%</div>
                                <div style={{ color:C.dim, fontSize:8, letterSpacing:"0.08em", fontFamily:C.fontBody, fontWeight:700, marginTop:2 }}>LIKE RATIO</div>
                              </div>
                              <div style={{ background:C.card, borderRadius:8, padding:"8px 10px", textAlign:"center" }}>
                                <div style={{ color:C.purple, fontSize:D.phone?14:16, fontWeight:400, fontFamily:C.fontHead }}>{fmt(v.likes)}</div>
                                <div style={{ color:C.dim, fontSize:8, letterSpacing:"0.08em", fontFamily:C.fontBody, fontWeight:700, marginTop:2 }}>LIKES</div>
                              </div>
                            </div>
                          </div>
                        );
  ...
  const GrowthView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {/* TikTok */}
      <div style={{ background:C.card, border:`1px solid ${C.borderStrong}`, borderRadius:14, padding:D.phone?"14px 16px":"18px 20px", borderTop:`1px solid ${C.pink}30` }}>
