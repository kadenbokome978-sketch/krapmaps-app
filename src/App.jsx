...
const Pill = ({ children, color, active, onClick }) => (
  <button onClick={onClick} style={{ padding:"7px 18px", borderRadius:20, border:`1px solid ${active?color:C.border}`, background:active?`${color}22`:"rgba(255,255,255,0.03)", color:active?color:"rgba(255,255,255,0.75)", fontSize:16, fontWeight:700, letterSpacing:"0.06em", cursor:"pointer", fontFamily:"'Lilita One', Georgia, serif", transition:"all 0.15s", whiteSpace:"nowrap", textTransform:"uppercase" }}>{children}</button>
);
...
const ActionBtn = ({ children, color, onClick }) => (
  <button onClick={onClick} style={{ padding:"5px 10px", borderRadius:8, background:`${color}18`, border:`1px solid ${color}30`, fontSize:16, fontWeight:600, color, cursor:"pointer", fontFamily:"'Lilita One', Georgia, serif", letterSpacing:"-0.01em", whiteSpace:"nowrap" }}>{children}</button>
);
...
const GlowAreaChart = ({ data=[], color=C.pink, height=120, dataKey="value", xKey="label" }) => {
  if(!data.length) return <div style={{height,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,color:"rgba(255,255,255,0.75)"}}>No data</div>;
...
const GlowBarChart = ({ data=[], color=C.pink, height=140, dataKey="value", xKey="label" }) => {
  if(!data.length) return <div style={{height,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"rgba(255,255,255,0.3)"}}>No data yet</div>;
...
const GlowLineChart = ({ data=[], color=C.cyan, height=100, dataKey="value", xKey="label" }) => {
  if(data.length<2) return <div style={{height,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,color:"rgba(255,255,255,0.75)"}}>Need more data</div>;
        ...
        <div style={{ position:"relative", padding:"44px 48px", display:"flex", alignItems:"stretch", gap:48 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
              ...
              <span style={{ fontSize:17, color:"rgba(255,255,255,0.7)", letterSpacing:"0.2em", textTransform:"uppercase", fontWeight:700 }}>Total Views All Time</span>
            </div>
            ...
            <div style={{ marginTop:24, display:"flex", alignItems:"center", gap:24 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <div style={{ width:8, height:8, borderRadius:"50%", background:scrapedStats?.scraped_at?C.green:"rgba(255,255,255,0.75)", boxShadow:scrapedStats?.scraped_at?`0 0 8px ${C.green}`:""  }}/>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          {[
            ...
            <button data-btn key={i} onClick={()=>runAI&&runAI(a.m)} disabled={aiLoad&&aiLoad[a.m]}
              style={{ display:"flex", alignItems:"center", gap:14, padding:"16px 18px", borderRadius:14, background:`linear-gradient(135deg,${a.c}0c,rgba(10,6,20,0.6))`, border:`1px solid ${a.c}1e`, cursor:"pointer", fontFamily:C.fontBody, opacity:aiLoad&&aiLoad[a.m]?0.5:1, transition:"all 0.2s", textAlign:"left", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", gap:8 }}>
          {["IDEAS","CALENDAR","CAPTIONS"].map(t=>(
            <button key={t} onClick={()=>setSub(t)} style={{ padding:"10px 20px", borderRadius:12, border:`1px solid ${sub===t?C.pink:"rgba(255,255,255,0.08)"}`, background:sub===t?`${C.pink}15`:"transparent", color:sub===t?C.pink:"rgba(255,255,255,0.5)", fontFamily:C.fontBody, fontWeight:700, fontSize:14, cursor:"pointer", letterSpacing:"0.06em" }}>
              {t}{t==="IDEAS"&&` (${ideas.length})`}{t==="CALENDAR"&&` (${calItems.length})`}
            </button>
          ))}
        </div>
        <button onClick={()=>openModal&&openModal(sub==="CALENDAR"?"addCal":"addIdea")} style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 20px", borderRadius:12, border:`1px solid ${C.pink}40`, background:`linear-gradient(135deg,${C.pink}20,${C.pink}08)`, color:C.pink, fontFamily:C.fontBody, fontWeight:700, fontSize:14, cursor:"pointer" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {["ALL","TIKTOK","INSTAGRAM","BOTH"].map(p=>(
              <button key={p} onClick={()=>setCalFilter(p)} style={{ padding:"8px 16px", borderRadius:10, border:`1px solid ${calFilter===p?C.cyan:"rgba(255,255,255,0.08)"}`, background:calFilter===p?`${C.cyan}15`:"transparent", color:calFilter===p?C.cyan:"rgba(255,255,255,0.45)", fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>
                {p}
              </button>
            ))}
          </div>
          ...
          {filteredCal.length===0
            ? <div style={{ padding:"48px", textAlign:"center", borderRadius:18, border:"1px solid rgba(255,255,255,0.07)", fontSize:16, color:"rgba(255,255,255,0.3)" }}>Nothing scheduled — tap Schedule to add</div>
...
Return ONLY JSON: {"overall_score":0-100,"performance_verdict":"viral|above_avg|average|below_avg|flopped","biggest_factor":"string","why_it_performed":"string","what_worked":["string"],"what_didnt":["string"],"replicate_these":["string"],"never_again":["string"],"refilm_brief":{"concept":"string","hook":"string","key_changes":["string"],"predicted_views":"string"}}`;
  ...
  const renderVidCard = (v,i) => {
                ...
                return (
                  <div key={v.id||i} style={{ borderRadius:16, background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.07)", overflow:"hidden", position:"relative" }}>
                    {sc && <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${sc.color},${sc.color}00)` }}/>}
                    ...
                    <div style={{ padding:"10px 18px", borderTop:"1px solid rgba(255,255,255,0.05)", display:"flex", gap:8, alignItems:"center" }}>
                      <button onClick={()=>{ setUpdateTarget&&setUpdateTarget(v); openModal&&openModal("updateVideo"); }} style={{ padding:"6px 14px", borderRadius:9, border:`1px solid ${C.cyan}30`, background:`${C.cyan}10`, color:C.cyan, fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>UPDATE</button>
                      <button onClick={()=>analyseVideo&&analyseVideo(v)} style={{ padding:"6px 14px", borderRadius:9, border:`1px solid ${C.purple}30`, background:vidAnalysis?.[v.id]?`${C.purple}25`:vidLoading?.[v.id]?`${C.purple}20`:`${C.purple}10`, color:C.purple, fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:vidLoading?.[v.id]?"wait":"pointer", opacity:vidLoading?.[v.id]?0.7:1, transition:"all 0.2s" }}>
                        {vidLoading?.[v.id] ? "⏳ ANALYSING..." : vidAnalysis?.[v.id] ? "✓ TEARDOWN" : "AI TEARDOWN"}
                      </button>
                      <button onClick={()=>deleteVideo&&deleteVideo(v.id)} style={{ marginLeft:"auto", padding:"6px 10px", borderRadius:9, border:`1px solid ${C.pink}20`, background:`${C.pink}08`, color:C.pink, fontFamily:C.fontBody, cursor:"pointer" }}>{I.trash(13,C.pink)}</button>
                    </div>
                    {/* Loading state */}
                    {vidLoading?.[v.id] && (
                      ...
                      return (
                        <div style={{ padding:"14px 18px", borderTop:`1px solid ${C.purple}20`, background:`${C.purple}06` }}>
                          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                            ...
                            <div style={{ flex:1, fontSize:14, color:"rgba(255,255,255,0.75)", lineHeight:1.4 }}>{ar.why_it_performed?.slice(0,100)}</div>
                          </div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"flex", gap:8 }}>
        {tabs.map(t=>(
          <button key={t} onClick={()=>setSub(t)} style={{ padding:"10px 20px", borderRadius:12, border:`1px solid ${sub===t?C.cyan:"rgba(255,255,255,0.08)"}`, background:sub===t?`${C.cyan}15`:"transparent", color:sub===t?C.cyan:"rgba(255,255,255,0.5)", fontFamily:C.fontBody, fontWeight:700, fontSize:14, cursor:"pointer", letterSpacing:"0.06em" }}>
            {t}
          </button>
        ))}
      </div>
      ...
      {sub==="OVERVIEW" && (()=>{
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ borderRadius:18, padding:"18px 22px", background:"linear-gradient(145deg,rgba(255,45,120,0.08),rgba(10,6,20,0.95))", border:"1px solid rgba(255,45,120,0.2)", position:"relative", overflow:"hidden" }}>
            <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.pink},${C.pink}00)` }}/>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
              {[
                ...
                <div key={i} style={{ padding:"14px 16px", borderRadius:14, background:`${s.c}10`, border:`1px solid ${s.c}20` }}>
                  ...
                  <div style={{ fontSize:32, fontWeight:400, fontFamily:C.fontHead, color:s.c, lineHeight:1, textShadow:`0 0 16px ${s.c}40` }}>{s.v}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ borderRadius:18, padding:"18px 22px", background:"linear-gradient(145deg,rgba(197,102,255,0.08),rgba(10,6,20,0.95))", border:"1px solid rgba(197,102,255,0.2)", position:"relative", overflow:"hidden" }}>
            <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.purple},${C.purple}00)` }}/>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            {/* TikTok top videos */}
            <div style={{ borderRadius:18, padding:"20px 22px", background:"rgba(255,255,255,0.025)", border:`1px solid ${C.pink}20`, position:"relative", overflow:"hidden" }}>
              <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.pink},${C.pink}00)` }}/>
            ...
            {/* Instagram top reels */}
            <div style={{ borderRadius:18, padding:"20px 22px", background:"rgba(255,255,255,0.025)", border:`1px solid ${C.purple}20`, position:"relative", overflow:"hidden" }}>
              <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.purple},${C.purple}00)` }}/>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:14 }}>
              ...
              {[{l:"TikTok",v:ttRatio,c:C.pink},{l:"Instagram",v:igRatio,c:C.purple}].map((p,i)=>(
                <div key={i} style={{ marginBottom:i===0?18:0 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                    <span style={{ fontSize:14, color:"rgba(255,255,255,0.7)", fontWeight:600 }}>{p.l}</span>
            ...
            {hookStats.length>0 ? (
                    ...
                    {hookStats.slice(0,5).map((h,i)=>(
                      <div key={h.hook} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"9px 0", borderBottom:i<4?"1px solid rgba(255,255,255,0.05)":"none" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                          ...
                          <span style={{ fontSize:14, color:i===0?"#fff":"rgba(255,255,255,0.7)", fontWeight:i===0?700:500 }}>{h.hook}</span>
                        </div>
              ...
              <div style={{ borderRadius:18, padding:"20px 22px", background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.07)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, color:"rgba(255,255,255,0.3)" }}>
      ...
      {sub==="VIDEOS" && (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {videos.length===0
            ? <div style={{ padding:"48px", textAlign:"center", fontSize:16, color:"rgba(255,255,255,0.3)", borderRadius:18, border:"1px solid rgba(255,255,255,0.07)" }}>No videos logged yet</div>
            : <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
              {/* TikTok column */}
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 16px", borderRadius:14, background:`${C.pink}10`, border:`1px solid ${C.pink}25` }}>
              ...
              {/* Instagram column */}
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 16px", borderRadius:14, background:`${C.purple}10`, border:`1px solid ${C.purple}25` }}>
                ...
                {videos.filter(v=>v.platform==="instagram").length===0 && (
                  <div style={{ padding:"32px", textAlign:"center", fontSize:14, color:"rgba(255,255,255,0.3)", borderRadius:14, border:"1px dashed rgba(255,255,255,0.1)" }}>No reels synced yet — hit SYNC NOW in Settings</div>
                )}
                {videos.filter(v=>v.platform==="instagram").map((v,i)=>renderVidCard(v,i))}
              </div>
            </div>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"flex", gap:8 }}>
        {["TO DO","APP IDEAS"].map(t=>(
          <button key={t} onClick={()=>setSub(t)} style={{ padding:"10px 22px", borderRadius:12, border:`1px solid ${sub===t?C.yellow:"rgba(255,255,255,0.08)"}`, background:sub===t?`${C.yellow}15`:"transparent", color:sub===t?C.yellow:"rgba(255,255,255,0.5)", fontFamily:C.fontBody, fontWeight:700, fontSize:14, cursor:"pointer", letterSpacing:"0.06em" }}>
      ...
      {sub==="TO DO" && (
        <div style={{ display:"grid", gridTemplateColumns:"3fr 2fr", gap:20, alignItems:"start" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            {/* Add task card */}
            <div style={{ borderRadius:18, padding:"20px 22px", background:"linear-gradient(145deg,rgba(255,213,10,0.08),rgba(10,6,20,0.9))", border:`1px solid ${C.yellow}25`, position:"relative", overflow:"hidden" }}>
              <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.yellow},${C.yellow}00)` }}/>
              ...
              <input
                ...
                style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, color:"#fff", padding:"14px 18px", fontSize:17, fontFamily:C.fontBody, outline:"none", marginBottom:14, boxSizing:"border-box" }}
              />
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                {/* Assignee picker */}
                <div style={{ display:"flex", gap:6 }}>
                  {["BK","HARLEY","BOTH"].map(a=>(
                    <button key={a} onClick={()=>setAssign(a)} style={{ padding:"8px 14px", borderRadius:10, border:`1px solid ${assign===a?ac(a):"rgba(255,255,255,0.1)"}`, background:assign===a?`${ac(a)}20`:"transparent", color:assign===a?ac(a):"rgba(255,255,255,0.45)", fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>
                      {a}
                    </button>
                  ))}
                </div>
                <button onClick={addTask} style={{ padding:"10px 22px", borderRadius:12, border:"none", background:`linear-gradient(135deg,${C.yellow},${C.orange})`, color:"#07050F", fontFamily:C.fontBody, fontWeight:700, fontSize:15, cursor:"pointer" }}>ADD</button>
              </div>
            </div>
            ...
            <div style={{ display:"flex", gap:6 }}>
              {assignees.map(a=>(
                <button key={a} onClick={()=>setTaskFilter(a)} style={{ flex:1, padding:"8px 6px", borderRadius:10, border:`1px solid ${taskFilter===a?ac(a==="ALL"?assign:a):"rgba(255,255,255,0.08)"}`, background:taskFilter===a?`${ac(a==="ALL"?assign:a)}15`:"transparent", color:taskFilter===a?ac(a==="ALL"?assign:a):"rgba(255,255,255,0.45)", fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer", textAlign:"center" }}>
                  {a}
                </button>
              ))}
            </div>
            ...
            {/* Pending tasks */}
            <div style={{ borderRadius:18, overflow:"hidden", border:"1px solid rgba(255,255,255,0.07)", background:"rgba(255,255,255,0.02)", minHeight:300 }}>
              {pending.length===0
                ? <div style={{ padding:"32px 20px", textAlign:"center", fontSize:15, color:"rgba(255,255,255,0.25)", fontStyle:"italic" }}>All clear — nothing pending</div>
                : pending.map((t,i)=>(
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            {/* Assignee breakdown */}
            <div style={{ borderRadius:16, padding:"16px 18px", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.07)" }}>
            ...
            {/* Completed */}
            {done.length>0 && (
              <div style={{ borderRadius:18, overflow:"hidden", border:"1px solid rgba(255,255,255,0.06)", background:"rgba(255,255,255,0.015)" }}>
                ...
                {done.slice(0,20).map((t,i)=>(
                  <div key={t.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"16px 22px", borderBottom:i<Math.min(done.length,20)-1?"1px solid rgba(255,255,255,0.04)":"none", opacity:0.55 }}>
      ...
      {sub==="APP IDEAS" && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {/* Add idea */}
          <div style={{ borderRadius:18, padding:"20px 22px", background:"linear-gradient(145deg,rgba(197,102,255,0.08),rgba(10,6,20,0.9))", border:`1px solid ${C.purple}25`, position:"relative", overflow:"hidden" }}>
            <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.purple},${C.purple}00)` }}/>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <input
                ...
                style={{ flex:1, background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, color:"#fff", padding:"14px 18px", fontSize:17, fontFamily:C.fontBody, outline:"none" }}
              />
              <button onClick={addIdea} style={{ padding:"12px 22px", borderRadius:12, border:"none", background:`linear-gradient(135deg,${C.purple},${C.pink})`, color:"#fff", fontFamily:C.fontBody, fontWeight:700, fontSize:15, cursor:"pointer", whiteSpace:"nowrap" }}>ADD</button>
            </div>
          </div>
              ...
              : appIdeas.map(idea=>{
                ...
                return (
                  <div key={idea.id} style={{ borderRadius:18, padding:"20px 22px", background:"rgba(255,255,255,0.025)", border:`1px solid ${sc_c}25`, position:"relative", overflow:"hidden" }}>
                    ...
                    <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                      <button onClick={()=>{ setEditAppIdeaTarget&&setEditAppIdeaTarget(idea); setModals&&setModals(m=>({...m,editAppIdea:true})); }} style={{ padding:"8px 14px", borderRadius:10, border:`1px solid ${C.yellow}30`, background:`${C.yellow}10`, color:C.yellow, fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>EDIT</button>
                      <button onClick={()=>setTasks(ts=>[{id:Date.now(),text:idea.text,assignee:"BK",done:false},...ts])} style={{ padding:"8px 14px", borderRadius:10, border:`1px solid ${C.cyan}30`, background:`${C.cyan}10`, color:C.cyan, fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>→ TO DO</button>
                      <button onClick={()=>setAppIdeas(is=>is.filter(x=>x.id!==idea.id))} style={{ padding:"8px 12px", borderRadius:10, border:`1px solid ${C.pink}20`, background:`${C.pink}08`, color:C.pink, fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>{I.trash(13,C.pink)}</button>
                    </div>
                  </div>
                );
              })
...
Return ONLY JSON: { ideas:[{title,hook,description,why_viral,why_beats_average,score,type,platform,cta,estimated_views,data_source:"live|channel_data|both"}] }`;
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      {/* Header */}
      <div style={{ borderRadius:20, padding:"24px 28px", background:`linear-gradient(135deg,${C.purple}25,${C.cyan}10,rgba(7,5,15,0.95))`, border:`1px solid ${C.purple}40`, position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:16, color:`${C.purple}cc`, marginTop:2 }}>Powered by Perplexity live search + Claude strategy</div>
          </div>
        </div>
        ...
        {!hasPPX && (
          <div style={{ marginTop:12, padding:"10px 14px", borderRadius:12, background:`${C.yellow}15`, border:`1px solid ${C.yellow}35`, fontSize:16, color:C.yellow }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
        {[
          ...
          <button key={i} data-btn onClick={btn.action} disabled={btn.key&&loading[btn.key]} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10, padding:"20px 12px", borderRadius:16, background:`linear-gradient(145deg,${btn.color}18,${btn.color}06)`, border:`1px solid ${btn.color}35`, cursor:"pointer", fontFamily:C.fontBody, opacity:btn.key&&loading[btn.key]?0.6:1, transition:"all 0.2s", position:"relative", overflow:"hidden" }}>
            <div style={{ width:52, height:52, borderRadius:14, background:`${btn.color}22`, border:`1px solid ${btn.color}45`, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 0 20px ${btn.color}25` }}>{btn.icon(22,btn.color)}</div>
            <div style={{ textAlign:"center" }}>
              ...
              <div style={{ fontSize:17, color:"rgba(255,255,255,0.7)", marginTop:3 }}>{btn.desc}</div>
            </div>
          </button>
        ))}
      </div>
      ...
      {/* LIVE TRENDS */}
      {trends && (
        <div data-card style={{ borderRadius:20, padding:"24px", background:`linear-gradient(145deg,${C.cyan}12,rgba(7,5,15,0.95))`, border:`1px solid ${C.cyan}35`, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.cyan},${C.cyan}00)` }} />
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
          ...
          {trends.biggest_opportunity && (
            <div style={{ marginBottom:16, padding:"18px 20px", borderRadius:16, background:`linear-gradient(135deg,${C.green}18,${C.cyan}08)`, border:`1px solid ${C.green}40` }}>
              ...
              <div style={{ fontSize:16, color:"rgba(255,255,255,0.85)", marginBottom:12 }}>{trends.biggest_opportunity.why_now}</div>
              <div style={{ padding:"10px 14px", borderRadius:10, background:`${C.green}12`, border:`1px solid ${C.green}25`, marginBottom:8 }}>
                <div style={{ fontSize:16, color:C.green, fontWeight:700, letterSpacing:"0.1em", marginBottom:4 }}>SUGGESTED VIDEO</div>
          ...
          {trends.trends?.map((t,i)=>(
            <div key={i} style={{ marginBottom:12, padding:"16px 18px", borderRadius:14, background:`rgba(255,255,255,0.04)`, border:`1px solid ${C.cyan}20` }}>
  ...
  <div style={{display:"flex",gap:6}}>
              ...
              <div style={{ fontSize:16, color:"rgba(255,255,255,0.85)", marginBottom:10, lineHeight:1.5 }}>{t.why}</div>
              <div style={{ padding:"10px 14px", borderRadius:10, background:`${C.cyan}10`, border:`1px solid ${C.cyan}25`, marginBottom:8 }}>
                <div style={{ fontSize:16, color:C.cyan, fontWeight:700, letterSpacing:"0.1em", marginBottom:4 }}>HOOK FOR YOUR CHANNEL</div>
          ...
          {trends.sounds?.length>0 && (
            <>
              <div style={{ fontSize:17, fontWeight:700, color:C.cyan, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:10, marginTop:16 }}>TRENDING SOUNDS</div>
              <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
        ...
        <div data-card style={{ borderRadius:20, padding:"24px", background:`linear-gradient(145deg,${C.green}12,rgba(7,5,15,0.95))`, border:`1px solid ${C.green}35`, position:"relative", overflow:"hidden" }}>
          ...
          {strategy.channel_diagnosis && (
            <div style={{ marginBottom:20, padding:"16px 18px", borderRadius:14, background:"rgba(255,255,255,0.04)", border:`1px solid ${C.green}25` }}>
              <div style={{ fontSize:17, fontWeight:700, color:C.green, letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:12 }}>CHANNEL DIAGNOSIS</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:12 }}>
                <div style={{ padding:"10px 14px", borderRadius:10, background:`${C.green}10`, border:`1px solid ${C.green}20` }}>
                  ...
                  {strategy.channel_diagnosis.strengths?.map((s,i)=><div key={i} style={{ fontSize:16, color:"rgba(255,255,255,0.7)", marginBottom:3 }}>✓ {s}</div>)}
                </div>
                <div style={{ padding:"10px 14px", borderRadius:10, background:`${C.pink}10`, border:`1px solid ${C.pink}20` }}>
                  ...
                  {strategy.channel_diagnosis.weaknesses?.map((s,i)=><div key={i} style={{ fontSize:16, color:"rgba(255,255,255,0.7)", marginBottom:3 }}>✗ {s}</div>)}
                </div>
              </div>
          ...
          {strategy.weeks?.map((w,i)=>(
            <div key={i} style={{ marginBottom:14, borderRadius:14, overflow:"hidden", border:`1px solid ${C.green}20` }}>
              <div style={{ padding:"12px 16px", background:`${C.green}15`, display:"flex", alignItems:"center", gap:10 }}>
                <div style={{ width:32, height:32, borderRadius:10, background:`${C.green}25`, border:`1px solid ${C.green}40`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:17, fontWeight:700, color:C.green }}>W{w.week}</div>
                ...
                {w.videos?.map((v,j)=>(
                  <div key={j} style={{ display:"flex", gap:12, padding:"10px 0", borderBottom:j<w.videos.length-1?`1px solid rgba(255,255,255,0.05)`:"none" }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:15, color:"rgba(255,255,255,0.7)" }}>{v.why_it_works}</div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {strategy.kpis.map((k,i)=>(
                  <div key={i} style={{ padding:"12px 14px", borderRadius:12, background:`${C.green}08`, border:`1px solid ${C.green}20` }}>
                    <div style={{ fontSize:16, fontWeight:700, color:C.green }}>{k.target}</div>
                    <div style={{ fontSize:17, color:"rgba(255,255,255,0.8)", marginTop:3 }}>{k.metric}</div>
                  </div>
                ))}
              </div>
            </>
      ...
      {/* 10 VIDEO IDEAS */}
      {contentPlan && (
        <div data-card style={{ borderRadius:20, padding:"24px", background:`linear-gradient(145deg,${C.yellow}12,rgba(7,5,15,0.95))`, border:`1px solid ${C.yellow}35`, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.yellow},${C.yellow}00)` }} />
          ...
          {contentPlan.ideas?.map((idea,i)=>(
            <div key={i} style={{ marginBottom:12, padding:"16px 18px", borderRadius:14, background:"rgba(255,255,255,0.03)", border:`1px solid ${scoreColor(idea.score)}25`, position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, marginBottom:8 }}>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:16, color:C.yellow, fontStyle:"italic", marginBottom:8 }}>"{idea.hook}"</div>
              ...
              {idea.why_beats_average && <div style={{ fontSize:15, color:C.cyan, lineHeight:1.4 }}>📈 {idea.why_beats_average}</div>}
                ...
                <div style={{ textAlign:"center", flexShrink:0 }}>
                  ...
                  <div style={{ fontSize:16, color:"rgba(255,255,255,0.7)", letterSpacing:"0.1em", marginTop:4 }}>VIRAL</div>
                </div>
              </div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
      ...
      {/* ── PREDICTIVE SCORER ──────────────────────────────────── */}
      <div id="predict-section" data-card style={{ borderRadius:20, padding:"24px", background:`linear-gradient(145deg,${C.purple}15,rgba(7,5,15,0.95))`, border:`1px solid ${C.purple}40`, position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.purple},${C.purple}00)` }} />
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:14 }}>
          <input value={predictInput.title} onChange={e=>setPredictInput(p=>({...p,title:e.target.value}))} placeholder="Video concept / title..." style={{ background:"rgba(255,255,255,0.06)", border:`1px solid ${C.purple}40`, borderRadius:12, color:"#fff", padding:"12px 16px", fontSize:17, fontFamily:C.fontBody, outline:"none" }} />
          <input value={predictInput.hook} onChange={e=>setPredictInput(p=>({...p,hook:e.target.value}))} placeholder="Opening hook (first 3 seconds)..." style={{ background:"rgba(255,255,255,0.06)", border:`1px solid ${C.purple}40`, borderRadius:12, color:"#fff", padding:"12px 16px", fontSize:17, fontFamily:C.fontBody, outline:"none" }} />
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <select value={predictInput.type} onChange={e=>setPredictInput(p=>({...p,type:e.target.value}))} style={{ background:"rgba(255,255,255,0.06)", border:`1px solid ${C.purple}40`, borderRadius:12, color:"#fff", padding:"12px 16px", fontSize:17, fontFamily:C.fontBody, outline:"none" }}>
              {["facecam","street","screencap","voiceover","mixed"].map(t=><option key={t} value={t} style={{background:"#0D0B18"}}>{t}</option>)}
            </select>
            <select value={predictInput.platform} onChange={e=>setPredictInput(p=>({...p,platform:e.target.value}))} style={{ background:"rgba(255,255,255,0.06)", border:`1px solid ${C.purple}40`, borderRadius:12, color:"#fff", padding:"12px 16px", fontSize:17, fontFamily:C.fontBody, outline:"none" }}>
              {["tiktok","instagram","both"].map(t=><option key={t} value={t} style={{background:"#0D0B18"}}>{t}</option>)}
            </select>
          </div>
          <button data-btn onClick={runPredict} disabled={loading.predict||!predictInput.title.trim()} style={{ padding:"14px", borderRadius:12, border:"none", background:`linear-gradient(135deg,${C.purple},${C.pink})`, color:"#fff", fontFamily:C.fontBody, fontWeight:700, fontSize:15, cursor:"pointer", opacity:loading.predict?0.6:1 }}>
        ...
        {predictResult && (
          <div style={{ marginTop:4 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:16 }}>
              {[
                ...
                <div key={i} style={{ textAlign:"center", padding:"14px 8px", background:`${s.c}10`, borderRadius:12, border:`1px solid ${s.c}25` }}>
                  ...
                  <div style={{ fontSize:16, color:"rgba(255,255,255,0.7)", letterSpacing:"0.1em", marginTop:6, textTransform:"uppercase" }}>{s.l}</div>
                </div>
              ))}
            </div>
            {/* Reasoning */}
            <div style={{ padding:"14px 16px", borderRadius:12, background:"rgba(255,255,255,0.04)", border:`1px solid rgba(255,255,255,0.08)`, marginBottom:12 }}>
              ...
              <div style={{ fontSize:17, color:"rgba(255,255,255,0.7)", lineHeight:1.6 }}>{predictResult.reasoning}</div>
            </div>
            {/* Will work / will fail */}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:12 }}>
              <div style={{ padding:"12px 14px", borderRadius:12, background:`${C.green}08`, border:`1px solid ${C.green}20` }}>
                ...
                {predictResult.what_will_work?.map((w,i)=><div key={i} style={{ fontSize:16, color:"rgba(255,255,255,0.7)", marginBottom:4 }}>✓ {w}</div>)}
              </div>
              <div style={{ padding:"12px 14px", borderRadius:12, background:`${C.pink}08`, border:`1px solid ${C.pink}20` }}>
                ...
                {predictResult.what_will_fail?.map((w,i)=><div key={i} style={{ fontSize:16, color:"rgba(255,255,255,0.7)", marginBottom:4 }}>✗ {w}</div>)}
              </div>
            </div>
            {/* Improved hook */}
            ...
            {predictResult.variations?.length>0 && (
              <div style={{ marginBottom:12 }}>
                ...
                {predictResult.variations.map((v,i)=>(
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", borderRadius:10, background:"rgba(255,255,255,0.03)", border:`1px solid rgba(255,255,255,0.06)`, marginBottom:6 }}>
            ...
            <div style={{ padding:"14px 16px", borderRadius:12, background:"rgba(255,255,255,0.03)", border:`1px solid rgba(255,255,255,0.06)` }}>
              <div style={{ fontSize:15, color:"rgba(255,255,255,0.7)", marginBottom:8 }}>After posting, log actual views to train the AI:</div>
              <div style={{ display:"flex", gap:8 }}>
                <input value={outcomeInput[0]||""} onChange={e=>setOutcomeInput(o=>({...o,0:e.target.value}))} placeholder="Actual views..." style={{ flex:1, background:"rgba(255,255,255,0.06)", border:`1px solid rgba(255,255,255,0.12)`, borderRadius:10, color:"#fff", padding:"10px 14px", fontSize:17, fontFamily:C.fontBody, outline:"none" }} />
                <button onClick={()=>saveOutcome(0,outcomeInput[0]||0)} style={{ padding:"10px 16px", borderRadius:10, border:"none", background:C.green, color:"#000", fontFamily:C.fontBody, fontWeight:700, fontSize:16, cursor:"pointer" }}>LOG</button>
              </div>
            </div>
          </div>
        )}
                ...
                {patterns.dayPerf.slice(0,3).map((d,i)=>(
                  <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      ...
                      <div style={{ fontSize:15, color:i===0?C.cyan:"rgba(255,255,255,0.7)", width:50, textAlign:"right" }}>{d.avg?.toLocaleString()}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div data-card style={{ borderRadius:20, padding:"24px", background:`linear-gradient(145deg,${C.purple}12,rgba(7,5,15,0.95))`, border:`1px solid ${C.purple}35`, position:"relative", overflow:"hidden" }}>
          ...
          {consensus.bothSucceeded && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
              {[{label:"CLAUDE SAYS", data:consensus.claude, color:C.purple},{label:"GPT-4O SAYS", data:consensus.gpt, color:C.cyan}].map((m,mi)=>(
                <div key={mi} style={{ padding:"14px 16px", borderRadius:14, background:`${m.color}08`, border:`1px solid ${m.color}20` }}>
          ...
          {consensus.bothSucceeded && (() => {
            ...
            return agreed.length > 0 ? (
              <div style={{ padding:"14px 16px", borderRadius:14, background:`${C.green}10`, border:`1px solid ${C.green}25`, marginBottom:12 }}>
          ...
          {!consensus.bothSucceeded && (consensus.claude||consensus.gpt)?.recommendations?.slice(0,5).map((r,i)=>(
            <div key={i} style={{ display:"flex", gap:12, padding:"12px 0", borderBottom:i<4?`1px solid rgba(255,255,255,0.05)`:"none" }}>
              <Tag color={r.priority==="CRITICAL"?C.pink:r.priority==="HIGH"?C.yellow:C.dim} sm>{r.priority}</Tag>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:17, fontWeight:700, color:"#fff", marginBottom:3 }}>{r.action}</div>
                <div style={{ fontSize:15, color:"rgba(255,255,255,0.75)" }}>{r.why}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {memory.entries?.length>0 && (
        <div data-card style={{ borderRadius:20, padding:"24px", background:`linear-gradient(145deg,${C.orange}10,rgba(7,5,15,0.95))`, border:`1px solid ${C.orange}30`, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.orange},${C.orange}00)` }} />
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
          ...
          {[...memory.entries].reverse().slice(0,10).map((e,i)=>(
            <div key={e.id||i} style={{ display:"flex", gap:12, padding:"10px 0", borderBottom:i<9?`1px solid rgba(255,255,255,0.05)`:"none" }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:16, color:"rgba(255,255,255,0.75)", lineHeight:1.4 }}>{e.recommendation}</div>
                {e.outcome && <div style={{ fontSize:15, color:C.green, marginTop:3 }}>→ {e.outcome}</div>}
                <div style={{ fontSize:17, color:"rgba(255,255,255,0.8)", marginTop:3 }}>{e.date}</div>
              </div>
            </div>
          ))}
...
Return ONLY JSON: {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      {/* Header */}
      <div style={{ borderRadius:20, padding:"24px 28px", background:`linear-gradient(135deg,${C.pink}20,${C.purple}10,rgba(7,5,15,0.95))`, border:`1px solid ${C.pink}40`, position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:16, color:`${C.pink}cc`, marginTop:2 }}>
        ...
        {(!hasGemini||!hasAnthrop) && (
          <div style={{ marginTop:14, padding:"10px 14px", borderRadius:12, background:`${C.yellow}15`, border:`1px solid ${C.yellow}35`, fontSize:16, color:C.yellow }}>
              ...
              {res && !isLoading && (
                <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:17, color:"rgba(255,255,255,0.7)", marginTop:3 }}>tap to see teardown</div>
                  </div>
                </div>
              )}
              {!res && !isLoading && (
      ...
      {/* Full teardown panel */}
      {result && selected && (
        <div data-card style={{ borderRadius:20, padding:"24px", background:`linear-gradient(145deg,${C.pink}10,rgba(7,5,15,0.95))`, border:`1px solid ${C.pink}35`, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.pink},${C.purple},${C.pink}00)` }} />
          ...
          <div style={{ fontSize:16, color:"rgba(255,255,255,0.7)", marginBottom:20 }}>Analysed {result.analysed_at?.slice(0,10)}</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:20 }}>
            {[
              ...
              <div key={i} style={{ padding:"14px", borderRadius:12, background:`${s.c}10`, border:`1px solid ${s.c}25`, textAlign:"center" }}>
                ...
                <div style={{ fontSize:16, color:"rgba(255,255,255,0.7)", letterSpacing:"0.1em", textTransform:"uppercase" }}>{s.l}</div>
              </div>
            ))}
          </div>
          ...
          {result.why_it_performed && (
            <div style={{ padding:"14px 16px", borderRadius:12, background:"rgba(255,255,255,0.04)", border:`1px solid rgba(255,255,255,0.08)`, marginBottom:14 }}>
              ...
              <div style={{ fontSize:17, color:"rgba(255,255,255,0.75)", lineHeight:1.6 }}>{result.why_it_performed}</div>
              {result.biggest_factor && <div style={{ marginTop:8, fontSize:16, color:C.yellow }}>⚡ Biggest factor: {result.biggest_factor}</div>}
            </div>
          )}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
            {result.replicate_these?.length>0 && (
              <div style={{ padding:"12px 14px", borderRadius:12, background:`${C.green}08`, border:`1px solid ${C.green}20` }}>
                ...
                {result.replicate_these.map((r,i)=><div key={i} style={{ fontSize:16, color:"rgba(255,255,255,0.7)", marginBottom:4 }}>✓ {r}</div>)}
              </div>
            )}
            {result.never_again?.length>0 && (
              <div style={{ padding:"12px 14px", borderRadius:12, background:`${C.pink}08`, border:`1px solid ${C.pink}20` }}>
                ...
                {result.never_again.map((r,i)=><div key={i} style={{ fontSize:16, color:"rgba(255,255,255,0.7)", marginBottom:4 }}>✗ {r}</div>)}
              </div>
            )}
          </div>
          ...
          {/* Refilm brief */}
          {result.refilm_brief && (
            <div style={{ padding:"16px 18px", borderRadius:14, background:`linear-gradient(135deg,${C.purple}12,${C.pink}06)`, border:`1px solid ${C.purple}30`, marginBottom:14 }}>
        ...
        <div key={plat.id} style={{ borderRadius:22, overflow:"hidden", background:`linear-gradient(145deg,${plat.color}12,rgba(10,6,20,0.95))`, border:`1px solid ${plat.color}30`, position:"relative" }}>
          ...
          <div style={{ padding:"24px 28px 20px", display:"flex", alignItems:"center", gap:16 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 14px", borderRadius:20, background:plat.live?`${C.green}12`:"rgba(255,255,255,0.05)", border:`1px solid ${plat.live?C.green:"rgba(255,255,255,0.1)"}` }}>
              <div style={{ width:6, height:6, borderRadius:"50%", background:plat.live?C.green:"rgba(255,255,255,0.3)", boxShadow:plat.live?`0 0 6px ${C.green}`:""  }}/>
...
const SettingsView = ({ keys, onEditKeys, scrapedStats, hasIG, WL, onEditWL, onSyncTikTok, syncMsg }) => {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ borderRadius:20, padding:"20px 24px", background:"linear-gradient(145deg,rgba(0,207,255,0.1),rgba(10,6,20,0.95))", border:`1px solid ${C.cyan}30`, display:"flex", alignItems:"center", justifyContent:"space-between", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.cyan},${C.cyan}00)` }}/>
        <div>
        ...
        <button onClick={onSyncTikTok} style={{ display:"flex", alignItems:"center", gap:10, padding:"14px 28px", borderRadius:14, border:`1px solid ${C.cyan}50`, background:`linear-gradient(135deg,${C.cyan}30,${C.cyan}15)`, color:C.cyan, fontFamily:C.fontBody, fontWeight:700, fontSize:16, cursor:"pointer", boxShadow:`0 0 24px ${C.cyan}20` }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, alignItems:"start" }}>
        ...
        {/* LEFT — API Keys */}
        <div style={{ borderRadius:20, padding:"22px 24px", background:"linear-gradient(145deg,rgba(255,45,120,0.07),rgba(10,6,20,0.95))", border:`1px solid ${C.pink}25`, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.pink},${C.pink}00)` }}/>
          <div style={{ fontSize:16, fontWeight:700, color:"#fff", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:20 }}>API Keys</div>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {apiKeys.map(k=>(
              <div key={k.id} style={{ borderRadius:14, padding:"14px 16px", background:"rgba(255,255,255,0.03)", border:`1px solid ${keys?.[k.id]?k.color+"30":"rgba(255,255,255,0.07)"}`, transition:"all 0.2s" }}>
                ...
                {editing===k.id && (
                  <div style={{ display:"flex", gap:8 }}>
                    ...
                    <button onClick={()=>saveKey(k.id)} style={{ padding:"10px 16px", borderRadius:10, border:"none", background:k.color, color:"#07050F", fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>SAVE</button>
                    <button onClick={()=>setEditing(null)} style={{ padding:"10px 14px", borderRadius:10, border:"1px solid rgba(255,255,255,0.1)", background:"transparent", color:"rgba(255,255,255,0.5)", fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>✕</button>
                  </div>
                )}
              </div>
            ))}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {/* System Status */}
          <div style={{ borderRadius:20, padding:"22px 24px", background:"linear-gradient(145deg,rgba(0,255,148,0.06),rgba(10,6,20,0.95))", border:`1px solid ${C.green}25`, position:"relative", overflow:"hidden" }}>
            <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.green},${C.green}00)` }}/>
            <div style={{ fontSize:16, fontWeight:700, color:"#fff", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:16 }}>System Status</div>
            {statusItems.map((s,i)=>(
              <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"11px 0", borderBottom:i<statusItems.length-1?"1px solid rgba(255,255,255,0.05)":"none" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  <span style={{ fontSize:14, color:"rgba(255,255,255,0.7)", fontWeight:500 }}>{s.label}</span>
                </div>
          ...
          {/* Creator Config */}
          <div style={{ borderRadius:20, padding:"22px 24px", background:"linear-gradient(145deg,rgba(197,102,255,0.07),rgba(10,6,20,0.95))", border:`1px solid ${C.purple}25`, position:"relative", overflow:"hidden" }}>
            <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${C.purple},${C.purple}00)` }}/>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
              <div style={{ fontSize:16, fontWeight:700, color:"#fff", letterSpacing:"0.08em", textTransform:"uppercase" }}>Creator Config</div>
              {!wlDraft && <button onClick={startWL} style={{ padding:"8px 16px", borderRadius:10, border:`1px solid ${C.purple}40`, background:`${C.purple}15`, color:C.purple, fontFamily:C.fontBody, fontWeight:700, fontSize:13, cursor:"pointer" }}>EDIT</button>}
            </div>
            {wlDraft ? (
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {[
                  ...
                  <div key={k}>
                    ...
                    {big ? (
                      <textarea value={wlDraft[k]||""} onChange={e=>setWlDraft(d=>({...d,[k]:e.target.value}))} rows={2}
                        style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:`1px solid ${C.purple}30`, borderRadius:10, color:"#fff", padding:"10px 12px", fontSize:14, fontFamily:C.fontBody, outline:"none", boxSizing:"border-box", resize:"vertical" }}/>
                    ) : (
                      <input value={wlDraft[k]||""} onChange={e=>setWlDraft(d=>({...d,[k]:e.target.value}))}
                        style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:`1px solid ${C.purple}30`, borderRadius:10, color:"#fff", padding:"10px 12px", fontSize:14, fontFamily:C.fontBody, outline:"none", boxSizing:"border-box" }}/>
                    )}
                  </div>
                ))}
                <div style={{ display:"flex", gap:8, marginTop:4 }}>
                  <button onClick={saveWLEdit} style={{ flex:1, padding:"12px", borderRadius:12, border:"none", background:C.purple, color:"#fff", fontFamily:C.fontBody, fontWeight:700, cursor:"pointer", fontSize:14 }}>SAVE CONFIG</button>
                  <button onClick={()=>setWlDraft(null)} style={{ padding:"12px 18px", borderRadius:12, border:"1px solid rgba(255,255,255,0.1)", background:"transparent", color:"rgba(255,255,255,0.5)", fontFamily:C.fontBody, fontWeight:700, cursor:"pointer" }}>CANCEL</button>
                </div>
              </div>
            ) : (
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {[
                  ...
                  <div key={k} style={{ padding:"10px 12px", background:"rgba(255,255,255,0.03)", borderRadius:10, border:`1px solid ${C.purple}15` }}>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100dvh - 160px)", minHeight:400 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:42, height:42, borderRadius:14, background:`linear-gradient(135deg,${C.pink},${C.purple})`, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 6px 24px ${C.pink}50, 0 0 0 1px ${C.pink}30` }}>
            {I.brain(20,"#fff")}
          </div>
          <div>
        ...
        {msgs.length > 1 && (
          <button onClick={clearChat} style={{ fontSize:11, padding:"6px 14px", borderRadius:10, border:`1px solid rgba(255,255,255,0.1)`, background:"rgba(255,255,255,0.04)", color:"rgba(255,255,255,0.35)", cursor:"pointer", fontFamily:C.fontBody, transition:"all 0.15s", letterSpacing:"0.04em" }}>
            End session
          </button>
        )}
      </div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {[
              ...
              <button key={s.label} onClick={()=>{ setInput(s.label); setTimeout(()=>inputRef.current?.focus(),50); }}
                style={{ display:"flex", alignItems:"center", gap:7, fontSize:12, padding:"9px 16px", borderRadius:14, border:`1px solid ${s.color}35`, background:`${s.color}10`, color:"rgba(255,255,255,0.75)", cursor:"pointer", fontFamily:C.fontBody, transition:"all 0.15s", letterSpacing:"0.02em" }}>
          ...
          <div key={i} style={{ display:"flex", flexDirection:"column", alignItems:msg.role==="user"?"flex-end":"flex-start", gap:10 }}>
            <div style={{ display:"flex", justifyContent:msg.role==="user"?"flex-end":"flex-start", gap:10, alignItems:"flex-end", maxWidth:"85%" }}>
              ...
              <div style={{
                ...
                color:"#fff", fontSize:13.5, lineHeight:1.7, fontFamily:C.fontBody, wordBreak:"break-word", whiteSpace:"pre-wrap",
            ...
            {msg.showCapcutBtn && (
              <button onClick={getCapcutPlan} disabled={loading}
                style={{ marginLeft:40, display:"flex", alignItems:"center", gap:8, padding:"11px 20px", borderRadius:14, border:`1px solid ${C.cyan}45`, cursor:loading?"not-allowed":"pointer", background:`linear-gradient(135deg,${C.cyan}14,${C.purple}14)`, color:C.cyan, fontSize:12, fontFamily:C.fontHead, fontWeight:700, opacity:loading?0.5:1, transition:"all 0.15s", letterSpacing:"0.04em", boxShadow:`0 4px 20px ${C.cyan}15` }}>
      ...
      <div style={{ display:"flex", gap:10, padding:"12px 14px", background:"rgba(255,255,255,0.05)", borderRadius:20, border:`1px solid rgba(255,255,255,0.1)`, marginTop:12, alignItems:"center", flexShrink:0, boxShadow:"0 4px 24px rgba(0,0,0,0.2)" }}>
        ...
        <input
          ...
          style={{ flex:1, background:"transparent", border:"none", color:"#fff", fontSize:14, fontFamily:C.fontBody, outline:"none", minWidth:0 }}
        />
  ...
  const ModalBase = ({ children, onClose }) => (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(8px)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"0 0 0 0" }}>
      <div style={{ background:"#0F0B1E",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,maxHeight:"88vh",overflowY:"auto",padding:"20px 18px 36px" }}>
        <div onClick={onClose} style={{ width:36,height:4,borderRadius:2,background:"rgba(255,255,255,0.75)",margin:"0 auto 20px" }} />
        {children}
      </div>
    </div>
  );
    ...
    return (
      <ModalBase onClose={()=>closeModal("addVideo")}>
        ...
        <div style={{ display:"flex",gap:6,marginBottom:16 }}>
          {["manual","scan"].map(t=><button key={t} onClick={()=>setTab(t)} style={{ flex:1,padding:"8px",borderRadius:10,border:`1px solid ${tab===t?C.pink:C.border}`,background:tab===t?C.pink+"20":"transparent",color:tab===t?C.pink:C.dim,fontFamily:C.fontBody,fontWeight:700,fontSize:17,cursor:"pointer",textTransform:"uppercase",letterSpacing:"0.06em" }}>{t}</button>)}
        </div>
        {tab==="manual" && (
          <>
    ...
    return (
      <ModalBase onClose={()=>closeModal("editIdea")}>
        ...
        <textarea
          ...
          rows={4}
          style={{ width:"100%",background:"rgba(255,255,255,0.06)",border:`1px solid ${C.pink}50`,borderRadius:12,color:C.text,padding:"12px 14px",fontSize:16,fontFamily:C.fontBody,outline:"none",boxSizing:"border-box",resize:"none",marginBottom:10,lineHeight:1.6 }}
        />
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
            {VIDEO_TYPES.map(t=>(
              <button key={t} onClick={()=>setType(t)} style={{ padding:"6px 12px",borderRadius:8,border:`1px solid ${type===t?C.pink:C.border}`,background:type===t?`${C.pink}20`:"transparent",color:type===t?C.pink:C.dim,fontFamily:C.fontBody,fontSize:17,fontWeight:700,cursor:"pointer",textTransform:"uppercase" }}>{t}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom:16 }}>
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8 }}>
            ...
            <button onClick={genHooks} disabled={loadingHooks||!title.trim()} style={{ padding:"5px 14px",borderRadius:8,border:`1px solid ${C.purple}40`,background:`${C.purple}18`,color:C.purple,fontFamily:C.fontBody,fontSize:16,fontWeight:700,cursor:"pointer",opacity:(loadingHooks||!title.trim())?0.4:1 }}>
          ...
          {displayHooks ? displayHooks.map((h,i)=>(
            ...
            <div style={{ padding:"14px",borderRadius:10,background:"rgba(255,255,255,0.03)",border:`1px solid ${C.border}`,textAlign:"center" }}>
    ...
    <div style={{ background:C.bg, minHeight:"100vh", fontFamily:C.fontBody, position:"relative" }}>
      ...
      <div className="web-sidebar">
        ...
        <div style={{ marginBottom:36, paddingLeft:4 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:40, height:40, borderRadius:14, background:`linear-gradient(135deg,${C.pink},${C.purple})`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:`0 0 20px ${C.pink}40` }}>
              {I.bin(18,"#fff")}
            </div>
            <div>
      ...
      <div className="web-content">
        <div className="web-inner" style={{ position:"relative", zIndex:1 }}>
          ...
          <div className="mobile-header" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 16px 0" }}>
            <div style={{ fontSize:24, fontWeight:900, color:"#fff", fontFamily:C.fontSora }}>Krap<span style={{color:C.pink}}>Maps</span></div>
          ...
          <div className="web-topbar" style={{ position:"sticky", top:0, zIndex:100, background:"rgba(6,4,14,0.92)", backdropFilter:"blur(40px)", WebkitBackdropFilter:"blur(40px)", borderBottom:"1px solid rgba(255,255,255,0.05)", padding:"0 40px", height:60, display:"flex", alignItems:"center", justifyContent:"space-between", boxShadow:"0 1px 0 rgba(255,255,255,0.04)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              ...
              <span style={{ fontSize:16, fontWeight:700, color:"rgba(255,255,255,0.8)", letterSpacing:"0.04em" }}>{NAV.find(n=>n.id===nav)?.label||nav}</span>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        {nav==="settings"  && <SettingsView keys={keys} onEditKeys={onEditKeys} scrapedStats={scrapedStats} hasIG={hasIG} WL={activeWL} onEditWL={onEditWL} onSyncTikTok={async()=>{
          ...
          {/* AI ERROR */}
        {aiErr && (
          <div style={{ position:"fixed", top:54, left:14, right:14, background:"rgba(10,5,20,0.98)", border:`1px solid ${C.pink}60`, borderRadius:14, padding:"12px 16px", color:"#FF8888", fontSize:15, zIndex:999, display:"flex", gap:10, alignItems:"flex-start", backdropFilter:"blur(20px)", boxShadow:`0 8px 32px rgba(0,0,0,0.6)` }}>
            ...
            <button onClick={()=>setAiErr(null)} style={{ background:"none",border:"none",color:"rgba(255,255,255,0.7)",cursor:"pointer",fontSize:18,lineHeight:1,flexShrink:0 }}>×</button>
          </div>
        )}
        </div>{/* end web-inner */}
      </div>{/* end web-content */}
