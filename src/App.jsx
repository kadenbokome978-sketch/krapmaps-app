...
const C = {
  ...
  muted:"#2D0840", mutedText:"#7A3A6A",
  text:"#F8EEFF", textMed:"#DDB0CC", dim:"#9A5080",
  // Fonts
...
const KEYS_KEY    = "krapmaps_v34_config";
const MANUAL_KEY  = "krapmaps_v34_manual";
const VIDEOS_KEY  = "krapmaps_v34_videos";
const IDEAS_KEY   = "krapmaps_v34_ideas";
const CAL_KEY     = "krapmaps_v34_calendar";
const TRENDS_KEY  = "krapmaps_v34_trends";
const TASKS_KEY   = "krapmaps_v34_tasks";
const APPIDEAS_KEY = "krapmaps_v34_appideas";

const SYNC_KEY   = "krapmaps_v34_syncurl";
...
function StatCard({ label, value, color, sub, onClick }) {
  return (
    <div onClick={onClick} style={{ background:`linear-gradient(135deg, ${C.card} 0%, ${color}08 100%)`, border:`1px solid ${color}30`, borderRadius:14, padding:"14px 16px", position:"relative", overflow:"hidden", cursor:onClick?"pointer":"default", boxShadow:`0 2px 12px ${color}12` }}>
      ...
      <div style={{ color:color+"99", fontSize:9, letterSpacing:"0.12em", fontFamily:C.fontBody, marginBottom:6, fontWeight:700 }}>{label}</div>
      <div style={{ color, fontSize:26, fontWeight:400, fontFamily:C.fontHead, lineHeight:1, textShadow:`0 0 16px ${color}50`, letterSpacing:"0.04em" }}>{value}</div>
...
function Btn({ children, onClick, color=C.pink, disabled=false, outline=false, small=false, full=false }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      fontFamily:C.fontBody, fontWeight:800, fontSize:small?11:13,
...
function FInput({ label, value, onChange, type="text", placeholder, textarea, rows=3, color=C.pink }) {
  ...
  const s = { width:"100%", background:C.cardAlt, border:`1px solid ${f?color+"80":C.border}`, borderRadius:10, color:C.text, padding:"10px 14px", fontSize:13, fontFamily:C.fontBody, outline:"none", boxSizing:"border-box", transition:"all 0.2s", boxShadow:f?`0 0 0 3px ${color}12`:"none" };
  return (
    <div>
...
function Modal({ title, onClose, children, maxWidth=500 }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div style={{ background:C.card, border:`1px solid ${C.borderStrong}`, borderRadius:18, padding:22, width:"100%", maxWidth, maxHeight:"90vh", overflowY:"auto", boxShadow:`0 20px 60px rgba(0,0,0,0.8), 0 0 0 1px ${C.borderStrong}` }} onClick={e=>e.stopPropagation()}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
          <div style={{ color:C.text, fontSize:17, fontWeight:800, fontFamily:C.fontBody }}>{title}</div>
...
function ScoreRing({ score, color, label, size=52 }) {
  const r=(size-8)/2, circ=2*Math.PI*r, scoreNorm=score>10?score/10:score, dash=(scoreNorm/10)*circ;
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
      <svg width={size} height={size}>
        ...
        <text x={size/2} y={size/2+4} textAnchor="middle" fill={color} fontSize="11" fontWeight="800" fontFamily="Bebas Neue, sans-serif">{score}</text>
      </svg>
...
function ChipGroup({ label, options, value, onChange, color=C.pink }) {
  return (
    <div>
      {label && <div style={{ color:C.mutedText, fontSize:9, letterSpacing:"0.12em", marginBottom:6, fontFamily:C.fontBody, fontWeight:700 }}>{label}</div>}
      <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
        {options.map(o => (
          <button key={o} onClick={()=>onChange(o)} style={{ padding:"5px 10px", borderRadius:7, cursor:"pointer", fontSize:10, background:value===o?color+"20":"transparent", border:`1px solid ${value===o?color:C.border}`, color:value===o?color:C.dim, fontFamily:C.fontBody, fontWeight:600, letterSpacing:"0.02em" }}>{o}</button>
        ))}
      </div>
    </div>
  );
    ...
    <div style={{ minHeight:"100vh", background:`linear-gradient(180deg,${C.pinkBg} 0%,${C.bg} 100%)`, display:"flex", alignItems:"center", justifyContent:"center", padding:24, minHeight:"100vh" }}>
      ...
      <div style={{ width:"100%", maxWidth:580 }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          ...
          <div style={{ color:C.pink, fontSize:28, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em", display:"flex", alignItems:"center", gap:8 }}><span style={{ color:C.pink }}>{Icon.bin}</span>KrapMaps</div><div style={{ color:C.textMed, fontSize:15, fontWeight:700, fontFamily:C.fontBody, marginTop:4 }}>Content OS</div>
          <div style={{ color:C.mutedText, fontSize:11, letterSpacing:"0.12em", fontFamily:C.fontBody, marginTop:6 }}>CONNECT YOUR PLATFORMS</div>
...
function Dashboard({ keys, onEditKeys }) {
  ...
  useEffect(()=>{ saveJSON(VIDEOS_KEY,videos); if(sbLoaded) sbSet("km_videos",videos); },[videos, sbLoaded]);
  ...
  useEffect(()=>{
    async function loadFromSupabase() {
      try { const [v,i,c,m,t] = await Promise.all([
  ...
  async function scoreIdea(idea) {
    ...
    try {
      const r = await callAI(`Score this KrapMaps TikTok idea. Return: {"viralityScore":0.0-10.0 (one decimal, e.g. 7.3),"hookScore":0.0-10.0 (one decimal, e.g. 8.1),"viralityReason":"1 sentence","hookFeedback":"1 sentence","improvedHook":"better hook in one line","bestFormat":"facecam|street|screencap","bestHookType":"string"}. Idea:"${idea.title}" Hook:${idea.hook} Notes:${idea.notes||"none"}`);
  ...
  const navBtnStyle = (id) => ({
    ...
    padding:"8px 0 10px", flex:1, background:"none", border:"none", cursor:"pointer",
    color: nav===id ? C.pink : C.mutedText,
    fontFamily:C.fontBody, fontWeight:600, fontSize:8, letterSpacing:"0.06em",
  ...
  const HomeView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      {/* Top banner — FortDice style */}
      <div style={{ borderRadius:20, padding:"20px 22px", position:"relative", overflow:"hidden", background:"linear-gradient(135deg,#120020 0%,#1E0035 50%,#120020 100%)", border:`1px solid ${C.pink}30`, boxShadow:`0 8px 40px ${C.pink}20` }}>
        {/* Decorative orb */}
        ...
        <div style={{ position:"absolute", bottom:-30, left:60, width:100, height:100, borderRadius:"50%", background:`radial-gradient(circle, ${C.purple}20 0%, transparent 70%)`, pointerEvents:"none" }} />
        <div style={{ position:"relative" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
            <div style={{ width:8, height:8, borderRadius:"50%", background:C.green, boxShadow:`0 0 8px ${C.green}` }} />
            <div style={{ color:C.dim, fontSize:10, fontFamily:C.fontBody, letterSpacing:"0.1em", fontWeight:600 }}>LIVE DASHBOARD</div>
          </div>
          <div style={{ color:C.text, fontSize:28, fontWeight:900, fontFamily:"'Sora', sans-serif", letterSpacing:"-0.02em", marginBottom:4 }}>KrapMaps <span style={{ color:C.pink, textShadow:`0 0 20px ${C.pink}` }}>HQ</span></div>
          <div style={{ color:C.dim, fontSize:12, fontFamily:C.fontBody }}>Bk + Harley · {new Date().toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"short"})}</div>
          <div style={{ marginTop:16, display:"flex", gap:10, alignItems:"center" }}>
            <div style={{ background:`linear-gradient(135deg, ${C.pink}, ${C.pinkDark})`, color:"#fff", fontFamily:C.fontHead, fontWeight:700, fontSize:12, padding:"6px 14px", borderRadius:20, boxShadow:`0 4px 16px ${C.pink}40` }}>Songkran T–{daysToSongkran}d</div>
            <div style={{ color:C.dim, fontSize:11, fontFamily:C.fontBody }}>April 13–15 · Thailand</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
        <div style={{ color:C.mutedText, fontSize:8, letterSpacing:"0.15em", fontFamily:C.fontBody, marginBottom:12, fontWeight:700 }}>QUICK ACTIONS</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          {[
            { icon:Icon.idea, label:"Add a video idea",    sub:"Save it before you forget",   action:()=>{ setNav("content"); setSub("ideas"); setTimeout(()=>openModal("addIdea"),50); } },
            { icon:Icon.calendar, label:"Schedule a post",     sub:"Add to the content calendar", action:()=>{ setNav("content"); setSub("calendar"); setTimeout(()=>openModal("addCal"),50); } },
            { icon:Icon.video, label:"Log a posted video",  sub:"Track its stats",             action:()=>openModal("addVideo") },
            { icon:Icon.tiktok, label:"Update TikTok stats", sub:"Followers, views, likes",     action:()=>openModal("editStats") },
          ].map(({ icon, label, sub, action }) => (
            <button key={label} onClick={action} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 14px", cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:10, transition:"all 0.2s" }}>
              <div style={{ width:36, height:36, borderRadius:10, background:C.pink+"18", border:`1px solid ${C.pink}25`, display:"flex", alignItems:"center", justifyContent:"center", color:C.pink, flexShrink:0 }}>{icon}</div>
              <div>
                <div style={{ color:C.text, fontSize:12, fontWeight:700, fontFamily:C.fontHead }}>{label}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              <div style={{ color:C.cyan, display:"flex" }}>{Icon.calendar}</div>
              <div style={{ color:C.text, fontSize:11, fontFamily:C.fontBody, fontWeight:700 }}>Upcoming</div>
            </div>
            <button onClick={()=>goTo("content","calendar")} style={{ background:"none", border:"none", color:C.cyan, fontSize:10, fontFamily:C.fontBody, cursor:"pointer", fontWeight:600 }}>View all</button>
          </div>
          {upcomingCal.length===0
            ? <div style={{ color:C.dim, fontSize:11, fontFamily:C.fontBody }}>Nothing scheduled — <span style={{ color:C.cyan, cursor:"pointer", fontWeight:600 }} onClick={()=>{ goTo("content","calendar"); openModal("addCal"); }}>add one</span></div>
            : upcomingCal.map(c=>(
              <div key={c.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, gap:6 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ color:C.text, fontSize:11, fontFamily:C.fontBody, fontWeight:600, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c.title}</div>
                  <div style={{ color:C.dim, fontSize:9, marginTop:1 }}>{c.date?.slice(5)} · {c.platform}</div>
                </div>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              <div style={{ color:C.purple, display:"flex" }}>{Icon.idea}</div>
              <div style={{ color:C.text, fontSize:11, fontFamily:C.fontBody, fontWeight:700 }}>Top Ideas</div>
            </div>
            <button onClick={()=>goTo("content","ideas")} style={{ background:"none", border:"none", color:C.purple, fontSize:10, fontFamily:C.fontBody, cursor:"pointer", fontWeight:600 }}>View all</button>
          </div>
          {topIdeas.length===0
            ? <div style={{ color:C.dim, fontSize:11, fontFamily:C.fontBody }}>No ideas yet — <span style={{ color:C.purple, cursor:"pointer", fontWeight:600 }} onClick={()=>{ goTo("content","ideas"); openModal("addIdea"); }}>add one</span></div>
            : topIdeas.slice(0,3).map(i=>(
              <div key={i.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, gap:6 }}>
                <div style={{ color:C.text, fontSize:11, fontFamily:C.fontBody, fontWeight:500, flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{i.title}</div>
                {i.aiScore && <div style={{ color:C.purple, fontFamily:C.fontHead, fontWeight:400, fontSize:14, flexShrink:0 }}>{i.aiScore.viralityScore}</div>}
              </div>
            ))
          }
        </div>
      ...
      {videos.length>=2 && (
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
          <div style={{ color:C.mutedText, fontSize:9, letterSpacing:"0.12em", fontFamily:C.fontBody, marginBottom:12 }}>AI ACTIONS</div>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {activeSub==="calendar" && (()=>{
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Content Calendar</div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              <div style={{ display:"flex", gap:6, background:C.card, padding:"5px", borderRadius:10, border:`1px solid ${C.border}` }}>
                {["all","TikTok","Instagram","Both"].map(p=>(
                  <button key={p} onClick={()=>setCalFilter(p)} style={{ flex:1, padding:"6px 10px", borderRadius:7, cursor:"pointer", fontSize:11, background:calFilter===p?C.pink:"transparent", border:"none", color:calFilter===p?"#fff":C.dim, fontFamily:C.fontBody, fontWeight:600, transition:"all 0.2s" }}>
                    {p==="all"?"All":p}
                  </button>
                ))}
              </div>
            ...
            {filteredCal.length===0
              ...
              : filteredCal.map(c=>(
                <div key={c.id} style={{ background:C.card, border:`1px solid ${c.date<today()?C.border:STATUS_C[c.status]+"44"}`, borderRadius:14, padding:"14px 16px", display:"grid", gridTemplateColumns:"80px 1fr auto auto", gap:12, alignItems:"center" }}>
                  <div>
                    <div style={{ color:STATUS_C[c.status]||C.dim, fontSize:14, fontWeight:800, fontFamily:C.fontBody }}>{c.date?.slice(5)}</div>
                    <div style={{ color:C.dim, fontSize:10 }}>{c.platform}</div>
                  </div>
                  <div>
                    <div style={{ color:C.text, fontSize:13, fontFamily:C.fontBody, fontWeight:600 }}>{c.title}</div>
                  ...
                  <select value={c.status} onChange={e=>updateCalStatus(c.id,e.target.value)} style={{ background:C.bg, border:`1px solid ${STATUS_C[c.status]||C.border}`, borderRadius:8, color:STATUS_C[c.status]||C.text, padding:"5px 8px", fontFamily:C.fontBody, fontWeight:700, fontSize:10, cursor:"pointer", outline:"none" }}>
        ...
        {activeSub==="ideas" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Idea Bank ({ideas.length})</div>
            ...
            {ideas.length===0
              ...
              : topIdeas.map(idea=>(
                <div key={idea.id} style={{ background:C.card, border:`1px solid ${(idea.aiScore?.viralityScore>=7||idea.aiScore?.viralityScore>=70)?C.pink+"44":C.border}`, borderRadius:16, padding:"16px 18px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10, marginBottom:10 }}>
                    <div style={{ flex:1 }}>
                      <div style={{ color:C.text, fontSize:14, fontWeight:800, fontFamily:C.fontBody, marginBottom:5 }}>{idea.title}</div>
                      <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                    ...
                    {idea.aiScore && (
                      <div style={{ display:"flex", gap:8, flexShrink:0 }}>
                        <ScoreRing score={idea.aiScore.viralityScore} color={(idea.aiScore.viralityScore>=7||idea.aiScore.viralityScore>=70)?C.green:(idea.aiScore.viralityScore>=5||idea.aiScore.viralityScore>=50)?C.yellow:C.dim} label="Viral" />
                        <ScoreRing score={idea.aiScore.hookScore} color={(idea.aiScore.hookScore>=7||idea.aiScore.hookScore>=70)?C.pink:(idea.aiScore.hookScore>=5||idea.aiScore.hookScore>=50)?C.orange:C.dim} label="Hook" />
                      </div>
                    )}
                  </div>
                  {idea.aiScore && (
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
                      <div style={{ background:C.pinkBg, borderRadius:8, padding:"10px 12px" }}>
                        <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.08em", fontFamily:C.fontBody, marginBottom:3 }}>VIRALITY</div>
                        <div style={{ color:C.text, fontSize:11, lineHeight:1.5 }}>{idea.aiScore.viralityReason}</div>
                      </div>
                      <div style={{ background:C.pinkBg, borderRadius:8, padding:"10px 12px" }}>
                        <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.08em", fontFamily:C.fontBody, marginBottom:3 }}>HOOK</div>
                      ...
                      {idea.aiScore.improvedHook && (
                        <div style={{ background:C.pink+"0F", border:`1px solid ${C.pink}33`, borderRadius:8, padding:"10px 12px", gridColumn:"1/-1" }}>
                          <div style={{ color:C.pink, fontSize:9, letterSpacing:"0.08em", fontFamily:C.fontBody, marginBottom:3 }}>✨ IMPROVED HOOK</div>
        ...
        {activeSub==="captions" && (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Caption Generator</div>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px 20px" }}>
              ...
              <div style={{ display:"flex", gap:10, marginBottom:12 }}>
                ...
                <Btn color={C.pink} onClick={()=>genCaption({title:draft.quickCaption||"KrapMaps video",hook:"gamification",notes:""})} disabled={aiLoad.caption}>{aiLoad.caption?"Generating...":"Generate Caption"}</Btn>
              </div>
              {ideas.length>0 && (
                <div>
                  <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.1em", marginBottom:7, fontFamily:C.fontBody }}>FROM IDEA BANK</div>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    {ideas.slice(0,8).map(i=>(
                      <button key={i.id} onClick={()=>genCaption(i)} style={{ background:C.muted+"22", border:`1px solid ${C.border}`, borderRadius:8, color:C.text, padding:"6px 12px", fontSize:11, fontFamily:C.fontBody, cursor:"pointer" }}>{i.title.slice(0,30)}{i.title.length>30?"...":""}</button>
                    ))}
                  </div>
                </div>
              )}
            ...
            {captionResult && captionIdea && (
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody }}>CAPTIONS FOR: {captionIdea.title}</div>
                {/* TikTok caption */}
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"16px 18px", borderTop:`1px solid ${C.pink}40` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                    <div style={{ color:C.pink, fontSize:12, fontWeight:800, fontFamily:C.fontBody }}>🎵 TikTok Caption</div>
                  ...
                  {(captionResult.tiktok?.altCaption||captionResult.altCaption) && (
                    <div style={{ background:C.pinkBg, borderRadius:8, padding:"10px 12px" }}>
                      <div style={{ color:C.mutedText, fontSize:9, fontFamily:C.fontBody, marginBottom:4 }}>EDGY ALT</div>
                ...
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px 20px", borderTop:`3px solid ${C.purple}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                    <div style={{ color:C.purple, fontSize:12, fontWeight:800, fontFamily:C.fontBody }}>📸 Instagram Caption</div>
                  ...
                  {captionResult.instagram?.altCaption && (
                    <div style={{ background:C.purple+"11", borderRadius:8, padding:"10px 12px" }}>
                      <div style={{ color:C.mutedText, fontSize:9, fontFamily:C.fontBody, marginBottom:4 }}>AESTHETIC ALT</div>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {activeSub==="videos" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Videos</div>
                  ...
                  {promoted.length>0 && (
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                      <div style={{ background:C.pinkBg, border:`1px solid ${C.borderStrong}`, borderRadius:12, padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        <div>
                          <div style={{ color:C.dim, fontSize:9, fontFamily:C.fontBody, letterSpacing:"0.1em", marginBottom:3 }}>ORGANIC AVG VIEWS</div>
                          <div style={{ color:C.pink, fontSize:18, fontWeight:800, fontFamily:C.fontBody }}>{fmt(organicAvgViews)}</div>
                      ...
                      <div style={{ background:"#F59E0B0F", border:`1px solid ${C.yellow}44`, borderRadius:12, padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        <div>
                          <div style={{ color:C.dim, fontSize:9, fontFamily:C.fontBody, letterSpacing:"0.1em", marginBottom:3 }}>PROMOTED AVG VIEWS</div>
                          <div style={{ color:C.yellow, fontSize:18, fontWeight:800, fontFamily:C.fontBody }}>{fmt(promotedAvgViews)}</div>
            ...
            {hookStats.length>0 && (
              <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
                <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.1em", marginBottom:12, fontFamily:C.fontBody }}>HOOK TYPE — AVG VIEWS</div>
                <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                  {hookStats.map((h,i)=>(
                    <div key={h.hook} style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <div style={{ color:i===0?C.yellow:C.dim, fontSize:10, width:130, fontFamily:C.fontBody, fontWeight:i===0?700:400 }}>{i===0&&"🏆 "}{h.hook}</div>
                      ...
                      <div style={{ color:i===0?C.yellow:C.text, fontSize:10, width:40, textAlign:"right", fontFamily:C.fontBody }}>{fmt(h.avg)}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {videos.length===0
              ...
              : (
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, overflow:"hidden" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 60px 70px 65px 55px 36px", padding:"8px 14px", background:C.pinkBg, color:C.dim, fontSize:9, fontWeight:700, letterSpacing:"0.1em", gap:6, fontFamily:C.fontBody }}>
                    {["TITLE","DATE","VIEWS","LIKES","RATIO",""].map(h=><div key={h}>{h}</div>)}
                  </div>
                  {sortedVideos.map((v,i)=>{
                    const r=ratio(v); const hot=r>=5;
                    return (
                      <div key={v.id} style={{ display:"grid", gridTemplateColumns:"1fr 60px 70px 65px 55px 36px", padding:"10px 14px", borderBottom:`1px solid ${C.border}`, gap:6, alignItems:"center", background:i===0?C.pinkBg:"transparent" }}>
                        <div>
                          <div style={{ color:C.text, fontSize:11, fontFamily:C.fontBody, fontWeight:600, marginBottom:3, lineHeight:1.2 }}>{i===0&&<span style={{ color:C.yellow, marginRight:4 }}>{Icon.star}</span>}{v.title}</div>
                        ...
                        <div style={{ color:C.cyan, fontWeight:700, fontFamily:C.fontBody, fontSize:11 }}>{fmt(v.views)}</div>
                        <div style={{ color:C.pink, fontWeight:700, fontFamily:C.fontBody, fontSize:11 }}>{fmt(v.likes)}</div>
                        <div style={{ color:hot?C.yellow:C.dim, fontWeight:hot?800:400, fontFamily:C.fontBody, fontSize:10 }}>{r.toFixed(1)}%</div>
                        <button onClick={()=>deleteVideo(v.id)} style={{ background:"#FF2D7815", border:"1px solid #FF2D7840", borderRadius:6, color:"#FF2D78", cursor:"pointer", padding:"3px 7px", display:"flex", alignItems:"center", justifyContent:"center" }}>{Icon.trash}</button>
                      </div>
                    );
                  })}
                </div>
        ...
        {activeSub==="analysis" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>What's Working</div>
            ...
            {!analysis
              ...
              : !analysis.keyPattern ? <div style={{ color:C.dim, padding:"20px", textAlign:"center", fontSize:13 }}>Unexpected format — tap Run Analysis again</div>
              : (
                <>
                  <div style={{ background:`linear-gradient(135deg,${C.pinkBg},${C.card})`, border:`1px solid ${C.pink}44`, borderRadius:14, padding:"18px 20px" }}>
                    <div style={{ color:C.pink, fontSize:9, letterSpacing:"0.12em", fontFamily:C.fontBody, marginBottom:5 }}>KEY PATTERN</div>
                    <div style={{ color:C.text, fontSize:15, fontWeight:800, fontFamily:C.fontBody, lineHeight:1.4 }}>{analysis.keyPattern}</div>
                    <div style={{ marginTop:12, padding:"11px 14px", background:C.yellow+"15", border:`1px solid ${C.yellow}44`, borderRadius:9 }}>
                      <div style={{ color:C.yellow, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:3 }}>⚡ URGENT FIX</div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    {[{label:"HOOK",data:analysis.hookAnalysis,bk:"bestHook",wk:"worstHook",c:C.cyan},{label:"FORMAT",data:analysis.typeAnalysis,bk:"bestType",wk:"worstType",c:C.green}].map(({label,data,bk,wk,c})=>(
                      <div key={label} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px" }}>
                        <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:8 }}>{label} ANALYSIS</div>
                  ...
                  {[{title:"What's Working",items:analysis.whatIsWorking,bg:C.green+"0A",border:C.green,icon:(i)=>i.impact==="high"?Icon.fire:i.impact==="medium"?Icon.growth:Icon.idea},{title:"Not Working",items:analysis.whatIsNotWorking,bg:"#FF2D2D08",border:"#FF2D2D",icon:()=>Icon.warning}].map(({title,items,bg,border:bc,icon})=>(
                    <div key={title} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"16px 18px" }}>
                      <div style={{ color:bc===C.green?C.green:"#FF6B6B", fontSize:13, fontWeight:800, fontFamily:C.fontBody, marginBottom:10 }}>{title}</div>
                      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                        {items?.map((item,i)=>(
                          <div key={i} style={{ display:"flex", gap:10, padding:"10px 12px", background:bg, border:`1px solid ${bc}22`, borderRadius:9 }}>
                            ...
                            <div style={{ flex:1 }}>
                              <div style={{ color:C.text, fontSize:11, fontWeight:700, fontFamily:C.fontBody, marginBottom:2 }}>{item.insight}</div>
        ...
        {activeSub==="nextVids" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Next 3 Videos</div>
            ...
            {!nextVids
              ...
              : (
                ...
                {(nextVids.tiktok||nextVids.nextVideos)?.length>0 && (
                  <div style={{ color:C.pink, fontSize:11, fontWeight:800, fontFamily:C.fontBody, marginBottom:4 }}>🎵 TikTok Videos</div>
                )}
                {(nextVids.tiktok||nextVids.nextVideos)?.map((v,i)=>(
                    <div key={i} style={{ background:C.card, border:`1px solid ${i===0?C.pink:C.border}`, borderRadius:14, padding:"18px 20px", borderTop:`3px solid ${i===0?C.pink:i===1?C.cyan:C.green}` }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10, gap:10 }}>
                        <div style={{ flex:1 }}>
                          <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:3 }}>{v.priority}</div>
                          <div style={{ color:C.text, fontSize:14, fontWeight:800, fontFamily:C.fontBody, lineHeight:1.3 }}>"{v.title}"</div>
                        </div>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                        <div style={{ background:C.pinkBg, borderRadius:9, padding:"10px 12px" }}>
                          <div style={{ color:C.yellow, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:3 }}>🎬 OPENING LINE</div>
                          <div style={{ color:C.text, fontSize:11, fontStyle:"italic", lineHeight:1.5 }}>"{v.openingLine}"</div>
                        </div>
                        <div style={{ background:C.pinkBg, borderRadius:9, padding:"10px 12px" }}>
                          <div style={{ color:C.cyan, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:3 }}>📐 STRUCTURE</div>
                      ...
                      <div style={{ marginTop:9, padding:"9px 12px", background:C.green+"0A", border:`1px solid ${C.green}22`, borderRadius:8 }}>
                        <div style={{ color:C.green, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:2 }}>WHY IT'LL WORK</div>
                  ...
                  {nextVids.songkranAngle && (
                    <div style={{ background:`linear-gradient(135deg,${C.pink}15,${C.yellow}08)`, border:`1px solid ${C.pink}44`, borderRadius:14, padding:"16px 20px" }}>
                      <div style={{ color:C.pink, fontSize:10, fontWeight:800, fontFamily:C.fontBody, marginBottom:6 }}>🎉 SONGKRAN SPECIAL</div>
        ...
        {activeSub==="weekly" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Harley Brief</div>
                ...
                <>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", gridColumn:"1/-1" }}>
                      <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:5 }}>WEEK SUMMARY</div>
                    ...
                    <div style={{ background:C.card, border:`1px solid ${C.green}33`, borderRadius:12, padding:"14px 16px" }}>
                      <div style={{ color:C.green, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:4 }}>🏆 TOP VIDEO</div>
                      <div style={{ color:C.text, fontSize:12, fontWeight:700, fontFamily:C.fontHead, marginBottom:4 }}>{weekly.topVideo?.title}</div>
                    ...
                    <div style={{ background:C.card, border:`1px solid ${C.cyan}33`, borderRadius:12, padding:"14px 16px" }}>
                      <div style={{ color:C.cyan, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:4 }}>💡 LEARNING</div>
                  ...
                  <div style={{ background:C.yellow+"0F", border:`1px solid ${C.yellow}44`, borderRadius:10, padding:"12px 16px", display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <div><div style={{ color:C.yellow, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:2 }}>THIS WEEK'S TARGET</div><div style={{ color:C.text, fontSize:12, fontWeight:700, fontFamily:C.fontHead }}>{weekly.thisWeekTarget}</div></div>
                  </div>
                  <div style={{ background:C.card, border:`1px solid ${C.pink}44`, borderRadius:12, padding:"16px 18px" }}>
                    <div style={{ color:C.pink, fontSize:10, fontWeight:800, fontFamily:C.fontBody, marginBottom:8 }}>📱 HARLEY'S FILMING BRIEF — BOTH PLATFORMS</div>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                      {weekly.tiktokHighlight && (
                        <div style={{ background:C.card, border:`1px solid ${C.pink}33`, borderRadius:12, padding:"14px 16px" }}>
                          <div style={{ color:C.pink, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:4 }}>🎵 TIKTOK HIGHLIGHT</div>
                          <div style={{ color:C.text, fontSize:12, fontWeight:700, fontFamily:C.fontHead, marginBottom:4 }}>{weekly.tiktokHighlight.title||weekly.topVideo?.title}</div>
                      ...
                      {weekly.instagramHighlight && (
                        <div style={{ background:C.card, border:`1px solid ${C.purple}33`, borderRadius:12, padding:"14px 16px" }}>
                          <div style={{ color:C.purple, fontSize:9, letterSpacing:"0.1em", fontFamily:C.fontBody, marginBottom:4 }}>📸 INSTAGRAM HIGHLIGHT</div>
                          <div style={{ color:C.text, fontSize:12, fontWeight:700, fontFamily:C.fontHead, marginBottom:4 }}>{weekly.instagramHighlight.caption}</div>
                  ...
                  <div style={{ background:C.card, border:`1px solid ${C.green}44`, borderRadius:12, padding:"16px 18px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                      <div style={{ color:C.green, fontSize:10, fontWeight:800, fontFamily:C.fontBody }}>💬 COPY FOR HARLEY (WhatsApp)</div>
                    ...
                    <div style={{ background:C.pinkBg, borderRadius:9, padding:"12px 14px", color:C.text, fontSize:12, lineHeight:1.8, fontFamily:C.fontMono, whiteSpace:"pre-wrap" }}>{weekly.rawSummaryText}</div>
                  </div>
                </>
              )
            }
        ...
        {activeSub==="trends" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Trend Analysis</div>
                ...
                <>
                  {trends.topOpportunity && (
                    <div style={{ background:`linear-gradient(135deg,#FFF7ED,${C.card})`, border:`1px solid ${C.orange}44`, borderRadius:14, padding:"18px 20px" }}>
                      <div style={{ color:C.orange, fontSize:9, letterSpacing:"0.12em", fontFamily:C.fontBody, marginBottom:5 }}>🔥 TOP OPPORTUNITY</div>
                      <div style={{ color:C.text, fontSize:15, fontWeight:800, fontFamily:C.fontBody, lineHeight:1.4 }}>{trends.topOpportunity}</div>
                    </div>
                  )}
                  {trends.songkranTrendAngle && (
                    <div style={{ background:`linear-gradient(135deg,${C.pink}15,${C.yellow}08)`, border:`1px solid ${C.pink}44`, borderRadius:12, padding:"14px 18px" }}>
                      <div style={{ color:C.pink, fontSize:10, fontWeight:800, fontFamily:C.fontBody, marginBottom:5 }}>🎉 SONGKRAN ANGLE</div>
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10, marginBottom:8 }}>
                          <div style={{ flex:1 }}>
                            <div style={{ display:"flex", gap:6, alignItems:"center", marginBottom:5 }}>
                              <div style={{ color:C.text, fontSize:13, fontWeight:800, fontFamily:C.fontBody }}>{t.trend}</div>
                            ...
                            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                              <div style={{ background:C.pinkBg, borderRadius:7, padding:"9px 11px" }}>
                                <div style={{ color:C.pink, fontSize:9, letterSpacing:"0.08em", fontFamily:C.fontBody, marginBottom:2 }}>🎵 TIKTOK ANGLE</div>
                              ...
                              {t.instagramAngle && (
                                <div style={{ background:C.purple+"0F", borderRadius:7, padding:"9px 11px" }}>
                                  <div style={{ color:C.purple, fontSize:9, letterSpacing:"0.08em", fontFamily:C.fontBody, marginBottom:2 }}>📸 INSTAGRAM ANGLE</div>
  ...
  const TasksView = () => {
    ...
    const addIdea = () => {
      ...
      setAppIdeas(is=>[{ id:Date.now(), text:ideaInput.trim(), category:"feature", created:today() }, ...is]);
      setIdeaInput("");
    };
    ...
    async function analyseAndRecommend() {
      ...
      try {
        ...
        const doneSummary = tasks.filter(t=>t.done).map(t=>t.text);
        const ideasSummary = appIdeas.map(i=>i.text);
        const result = await callAI(`You are advising KrapMaps (bin-finding gamified app) and its Content OS tool business. Analyse the current to-do list and app ideas, then recommend the next 3 most impactful ideas to add. Return: {"analysis":"2 sentence summary of current priorities and gaps","recommendations":[{"title":"","why":"1 sentence reason this is high impact","category":"feature|content|business|growth","assignee":"Bk|Harley|Both"}],"topPriority":"the single most important thing to do right now in one sentence"}. Current todos: ${JSON.stringify(todoSummary)}. Done: ${JSON.stringify(doneSummary)}. App ideas: ${JSON.stringify(ideasSummary)}.`, 1500);
        setAiRecs(result);
      } catch(e) { setAiRecErr(e.message); }
      setAiRecLoad(false);
    }
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        {/* Header */}
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ color:C.text, fontSize:20, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Tasks & Ideas</div>
        ...
        <div style={{ display:"flex", gap:4, background:C.card, padding:"5px", borderRadius:10, border:`1px solid ${C.border}` }}>
          {[["todo","To Do"],["ideas","App Ideas"],["done","Done"]].map(([id,label])=>(
            <button key={id} onClick={()=>setActiveTab(id)} style={{ flex:1, padding:"7px", borderRadius:7, border:"none", background:activeTab===id?C.pink:"transparent", color:activeTab===id?"#fff":C.dim, fontFamily:C.fontBody, fontWeight:600, fontSize:11, cursor:"pointer", transition:"all 0.2s" }}>{label} {id==="todo"&&pendingTasks.length>0?`(${pendingTasks.length})`:id==="done"&&doneTasks.length>0?`(${doneTasks.length})`:""}</button>
          ))}
        </div>
                ...
                <div key={i} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:"flex", gap:6, alignItems:"center", marginBottom:4 }}>
                      <div style={{ color:C.text, fontSize:13, fontWeight:700, fontFamily:C.fontBody }}>{r.title}</div>
                    ...
                    <div style={{ color:C.dim, fontSize:11, lineHeight:1.5 }}>{r.why}</div>
                  </div>
        ...
        {activeTab==="todo" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <input value={taskInput} onChange={e=>setTaskInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTask()} placeholder="Add a task..." style={{ flex:1, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, color:C.text, padding:"10px 14px", fontSize:13, fontFamily:C.fontBody, outline:"none", boxSizing:"border-box" }} />
              <div style={{ display:"flex", gap:6, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"4px" }}>
                {["Bk","Harley","Both"].map(a=>(
                  <button key={a} onClick={()=>setTaskAssign(a)} style={{ padding:"4px 10px", borderRadius:7, border:"none", background:taskAssign===a?C.pink:"transparent", color:taskAssign===a?"#fff":C.dim, fontFamily:C.fontBody, fontSize:10, fontWeight:600, cursor:"pointer" }}>{a}</button>
                ))}
              </div>
            ...
            {pendingTasks.length===0
              ? <div style={{ textAlign:"center", padding:"32px", color:C.dim, fontSize:13, fontFamily:C.fontBody }}>No tasks — you're clear 🎯</div>
              : pendingTasks.map(t=>(
                <div key={t.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px", display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ color:C.text, fontSize:13, fontFamily:C.fontBody, fontWeight:500 }}>{t.text}</div>
                    <div style={{ color:C.dim, fontSize:10, marginTop:2 }}>{t.created}</div>
                  </div>
        ...
        {activeTab==="ideas" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {/* Add idea */}
            <div style={{ display:"flex", gap:8 }}>
              <input value={ideaInput} onChange={e=>setIdeaInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addIdea()} placeholder="Add a KrapMaps feature idea..." style={{ flex:1, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, color:C.text, padding:"10px 14px", fontSize:13, fontFamily:C.fontBody, outline:"none", boxSizing:"border-box" }} />
              <Btn onClick={addIdea} disabled={!ideaInput.trim()}>{Icon.plus}</Btn>
            </div>
            {/* Ideas list */}
            {appIdeas.length===0
              ? <div style={{ textAlign:"center", padding:"32px", color:C.dim, fontSize:13, fontFamily:C.fontBody }}>No ideas yet — tap AI Recommend to get started</div>
              : appIdeas.map(idea=>(
                <div key={idea.id} style={{ background:C.card, border:`1px solid ${C.cyan}20`, borderRadius:12, padding:"12px 14px", display:"flex", alignItems:"flex-start", gap:12 }}>
                  <div style={{ width:8, height:8, borderRadius:"50%", background:C.cyan, marginTop:5, flexShrink:0, boxShadow:`0 0 8px ${C.cyan}` }} />
                  <div style={{ flex:1 }}>
                    <div style={{ color:C.text, fontSize:13, fontFamily:C.fontBody, fontWeight:500, lineHeight:1.4 }}>{idea.text}</div>
                    <div style={{ color:C.dim, fontSize:10, marginTop:3 }}>{idea.created}</div>
                  </div>
                  <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                    <button onClick={()=>{ setTasks(ts=>[{ id:Date.now(), text:idea.text, done:false, assignee:"Both", created:today() },...ts]); setActiveTab("todo"); }} style={{ background:C.pink+"15", border:`1px solid ${C.pink}30`, borderRadius:7, color:C.pink, cursor:"pointer", padding:"4px 10px", fontSize:10, fontFamily:C.fontBody, fontWeight:600 }}>→ Todo</button>
                    <button onClick={()=>setAppIdeas(is=>is.filter(x=>x.id!==idea.id))} style={{ background:"transparent", border:`1px solid ${C.border}`, borderRadius:7, color:C.dim, cursor:"pointer", padding:"3px 7px", display:"flex", alignItems:"center" }}>{Icon.trash}</button>
                  </div>
                </div>
              ))
            }
        ...
        {activeTab==="done" && (
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {doneTasks.length===0
              ? <div style={{ textAlign:"center", padding:"32px", color:C.dim, fontSize:13, fontFamily:C.fontBody }}>Nothing completed yet</div>
              : doneTasks.map(t=>(
                <div key={t.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px", display:"flex", alignItems:"center", gap:12, opacity:0.6 }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ color:C.text, fontSize:13, fontFamily:C.fontBody, fontWeight:500, textDecoration:"line-through" }}>{t.text}</div>
                  </div>
                  <Tag color={t.assignee==="Harley"?C.orange:t.assignee==="Both"?C.purple:C.pink} size={9}>{t.assignee}</Tag>
                  <button onClick={()=>setTasks(ts=>ts.filter(x=>x.id!==t.id))} style={{ background:"transparent", border:`1px solid ${C.border}`, borderRadius:7, color:C.dim, cursor:"pointer", padding:"3px 7px", display:"flex", alignItems:"center" }}>{Icon.trash}</button>
                </div>
  ...
  const GrowthView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Growth Stats</div>
        <Btn small outline color={C.pink} onClick={()=>openModal("editStats")}>Update Stats</Btn>
      </div>

      {/* TikTok */}
      <div style={{ background:C.card, border:`1px solid ${C.borderStrong}`, borderRadius:14, padding:"16px 18px", borderTop:`1px solid ${C.pink}40` }}>
        <div style={{ color:C.mutedText, fontSize:9, letterSpacing:"0.12em", fontFamily:C.fontBody, marginBottom:12, fontWeight:700 }}>TIKTOK (@findkrap)</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8 }}>
        ...
        {igData && (
          <>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:14 }}>
              ...
              <StatCard label="POSTS" value={fmt(igData.profile.media_count)} color={C.green} sub="total" />
            </div>
          </>
        )}
      </div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px 20px", borderTop:`3px solid ${C.cyan}` }}>
        <div style={{ color:C.mutedText, fontSize:9, letterSpacing:"0.12em", fontFamily:C.fontBody, marginBottom:12, fontWeight:700, display:"flex", alignItems:"center", gap:8 }}>APP STORE {hasApple&&<Tag color={C.green} size={8}>KEY SAVED</Tag>}</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          {[["iOS Downloads","ios_downloads","number"],["iOS Rating","ios_rating","text"],["iOS Reviews","ios_reviews","number"],["Active Users","ios_active","number"]].map(([label,key,type])=>(
            <div key={key}>
              <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.08em", marginBottom:3, fontFamily:C.fontBody }}>{label}</div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px 20px", borderTop:`3px solid ${C.green}` }}>
        <div style={{ color:C.mutedText, fontSize:9, letterSpacing:"0.12em", fontFamily:C.fontBody, marginBottom:12 }}>▶️ GOOGLE PLAY {hasPlay&&<Tag color={C.green} size={8}>KEY SAVED</Tag>}</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
  ...
  const SettingsView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.02em" }}>Settings</div>
      ...
      {/* API Keys */}
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px 20px" }}>
        <div style={{ color:C.mutedText, fontSize:8, letterSpacing:"0.15em", fontFamily:C.fontBody, marginBottom:12, fontWeight:700 }}>API KEYS</div>
  ...
  function Empty({ icon, title, sub, action, actionLabel, disabled=false, color=C.pink }) {
    return (
      <div style={{ background:C.pinkBg, border:`2px dashed ${C.borderStrong}`, borderRadius:16, padding:"44px 24px", textAlign:"center" }}>
        ...
        <div style={{ color:C.text, fontSize:14, fontWeight:800, fontFamily:C.fontBody, marginBottom:6 }}>{title}</div>
  ...
  const AddVideoModal = () => {
    ...
    const Toggle = ({field, label, sub, color}) => (
      <div style={{ background:form[field]?color+"11":C.pinkBg, border:`1px solid ${form[field]?color:C.border}`, borderRadius:12, padding:"11px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer" }} onClick={()=>set(field,!form[field])}>
        <div><div style={{ color:form[field]?color:C.dim, fontWeight:800, fontSize:12, fontFamily:C.fontBody }}>{label}</div><div style={{ color:C.dim, fontSize:9, marginTop:2 }}>{sub}</div></div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <div style={{ display:"flex", gap:5, background:C.pinkBg, padding:"5px", borderRadius:12, border:`1px solid ${C.borderStrong}` }}>
            {[["screenshot","📱 Scan Screenshot"],["manual","✏️ Manual Entry"]].map(([m,l])=>(
              <button key={m} onClick={()=>setMode(m)} style={{ flex:1, background:mode===m?C.pink:"transparent", border:"none", borderRadius:8, padding:"8px", color:mode===m?"#fff":C.mutedText, fontFamily:C.fontBody, fontWeight:800, fontSize:11, cursor:"pointer", transition:"all 0.15s" }}>{l}</button>
            ))}
          </div>
