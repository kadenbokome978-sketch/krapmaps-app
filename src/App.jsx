...
const KEYS_KEY    = "krapmaps_v41_config";
const MANUAL_KEY  = "krapmaps_v41_manual";
const VIDEOS_KEY  = "krapmaps_v41_videos";
const IDEAS_KEY   = "krapmaps_v41_ideas";
const CAL_KEY     = "krapmaps_v41_calendar";
const TRENDS_KEY  = "krapmaps_v41_trends";
const TASKS_KEY   = "krapmaps_v41_tasks";
const APPIDEAS_KEY = "krapmaps_v41_appideas";

const SYNC_KEY   = "krapmaps_v41_syncurl";
...
function StatCard({ label, value, color, sub, onClick }) {
  return (
    <div onClick={onClick} style={{ background:`linear-gradient(135deg, ${C.card} 0%, ${color}08 100%)`, border:`1px solid ${color}30`, borderRadius:14, padding:"14px 16px", position:"relative", overflow:"hidden", cursor:onClick?"pointer":"default", boxShadow:`0 2px 12px ${color}12` }}>
      ...
      <div style={{ color, fontSize:28, fontWeight:400, fontFamily:C.fontHead, lineHeight:1, textShadow:`0 0 16px ${color}40`, letterSpacing:"0.06em" }}>{value}</div>
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div style={{ background:C.card, border:`1px solid ${C.borderStrong}`, borderRadius:18, padding:22, width:"100%", maxWidth, maxHeight:"90vh", overflowY:"auto", boxShadow:`0 20px 60px rgba(0,0,0,0.8), 0 0 0 1px ${C.borderStrong}` }} onClick={e=>e.stopPropagation()}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
          <div style={{ color:C.text, fontSize:18, fontWeight:900, fontFamily:C.fontHead, letterSpacing:"-0.01em" }}>{title}</div>
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
        {options.map(o => (
          <button key={o} onClick={()=>onChange(o)} style={{ padding:"5px 10px", borderRadius:7, cursor:"pointer", fontSize:10, background:value===o?color+"20":"transparent", border:`1px solid ${value===o?color:C.border}`, color:value===o?color:"#fff", fontFamily:C.fontBody, fontWeight:600, letterSpacing:"0.02em", opacity:value===o?1:0.6, textTransform:"uppercase", fontSize:10 }}>{o}</button>
        ))}
      </div>
    </div>
  );
...
function Dashboard({ keys, onEditKeys }) {
  ...
  async function scoreIdea(idea) {
    ...
    try {
      const r = await callAI(`Score this KrapMaps app idea precisely. Return JSON only: {"appScore":0-100 (integer, precise e.g. 73 not 75),"viralityScore":0-100 (integer, precise e.g. 67 not 70),"hookScore":0-100 (integer, precise e.g. 81 not 80),"verdict":"one punchy sentence","viralityReason":"one sentence","hookFeedback":"one sentence","improvedHook":"better hook rewritten","bestFormat":"facecam|street|screencap","recommendations":[{"action":"specific thing to do","impact":"high|medium|low"},{"action":"specific thing to do","impact":"high|medium|low"},{"action":"specific thing to do","impact":"high|medium|low"}]}. Idea:"${idea.title}" Type:${idea.type} Hook:${idea.hook} Notes:${idea.notes||"none"}`, 1000);
  ...
  const HomeView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ borderRadius:20, padding:"20px 22px", position:"relative", overflow:"hidden", background:"linear-gradient(135deg,#120020 0%,#1E0035 50%,#120020 100%)", border:`1px solid ${C.pink}30`, boxShadow:`0 8px 40px ${C.pink}20` }}>
        ...
        <div style={{ position:"relative" }}>
          ...
          <div style={{ marginTop:16, display:"flex", gap:10, alignItems:"center" }}>
            <div style={{ background:`linear-gradient(135deg, ${C.pink}, ${C.pinkDark})`, color:"#fff", fontFamily:C.fontBody, fontWeight:600, fontSize:11, padding:"4px 12px", borderRadius:20, boxShadow:`0 4px 12px ${C.pink}40` }}>Songkran · {daysToSongkran}d · Apr 13–15</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"18px 20px" }}>
        <div style={{ color:C.dim, fontSize:8, letterSpacing:"0.15em", fontFamily:C.fontBody, marginBottom:12, fontWeight:700 }}>QUICK ACTIONS</div>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              ...
              <div style={{ color:C.text, fontSize:11, fontFamily:C.fontBody, fontWeight:700, letterSpacing:"0.05em", textTransform:"uppercase" }}>Upcoming</div>
            </div>
          ...
          {upcomingCal.length===0
            ? <div style={{ color:C.dim, fontSize:10, fontFamily:C.fontBody, letterSpacing:"0.06em" }}>Nothing scheduled — <span style={{ color:C.cyan, cursor:"pointer", fontWeight:700 }} onClick={()=>{ goTo("content","calendar"); openModal("addCal"); }}>add one</span></div>
            : upcomingCal.map(c=>(
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              ...
              <div style={{ color:C.text, fontSize:11, fontFamily:C.fontBody, fontWeight:700, letterSpacing:"0.05em", textTransform:"uppercase" }}>Top Ideas</div>
            </div>
          ...
          {topIdeas.length===0
            ? <div style={{ color:C.dim, fontSize:10, fontFamily:C.fontBody, letterSpacing:"0.06em" }}>No ideas yet — <span style={{ color:C.purple, cursor:"pointer", fontWeight:700 }} onClick={()=>{ goTo("content","ideas"); openModal("addIdea"); }}>add one</span></div>
            : topIdeas.slice(0,3).map(i=>(
              <div key={i.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, gap:6 }}>
                ...
                {i.aiScore && <div style={{ color:i.aiScore.appScore>=80?C.green:i.aiScore.appScore>=60?C.yellow:C.pink, fontFamily:C.fontHead, fontWeight:400, fontSize:16, flexShrink:0 }}>{i.aiScore.appScore??i.aiScore.viralityScore}</div>}
              </div>
            ))
          }
        </div>
      ...
      {videos.length>=2 && (
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"14px 16px" }}>
          <div style={{ color:C.dim, fontSize:9, letterSpacing:"0.12em", fontFamily:C.fontBody, marginBottom:12 }}>AI ACTIONS</div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            ...
            <Btn small outline color={C.orange} onClick={()=>runAI("trends")}   disabled={aiLoad.trends}>{aiLoad.trends?"Analysing...":"Trends"}</Btn>
          </div>
        </div>
      )}
    </div>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {activeSub==="calendar" && (()=>{
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>Content Calendar</div>
        ...
        {activeSub==="ideas" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>Idea Bank ({ideas.length})</div>
        ...
        {activeSub==="captions" && (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>Caption Generator</div>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div style={{ display:"flex", gap:4, overflowX:"auto", scrollbarWidth:"none" }}>
          {analyticsSubs.map(s=>(
            <button key={s.id} onClick={()=>setSub(s.id)} style={{ background:activeSub===s.id?C.pink:"transparent", border:`1px solid ${activeSub===s.id?C.pink:C.border}`, borderRadius:20, padding:"6px 16px", color:"#fff", fontFamily:C.fontBody, fontWeight:700, fontSize:11, cursor:"pointer", whiteSpace:"nowrap", opacity:activeSub===s.id?1:0.5, textTransform:"uppercase", letterSpacing:"0.05em" }}>{s.label}</button>
          ))}
        </div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                  {hookStats.map((h,i)=>(
                    <div key={h.hook} style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <div style={{ color:i===0?C.yellow:"#fff", fontSize:10, width:140, fontFamily:C.fontBody, fontWeight:i===0?700:500, textTransform:"uppercase", letterSpacing:"0.04em", opacity:i===0?1:0.8 }}>{i===0&&<span style={{color:C.yellow,marginRight:4}}>{Icon.star}</span>}{h.hook}</div>
            ...
            {videos.length===0
              ...
              : (
                <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, overflow:"hidden" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 55px 65px 60px 55px 36px", padding:"10px 16px", background:C.pinkBg, color:"#fff", fontSize:9, fontWeight:700, letterSpacing:"0.12em", gap:8, fontFamily:C.fontBody, opacity:0.7 }}>
                    {["TITLE","DATE","VIEWS","LIKES","RATIO",""].map(h=><div key={h}>{h}</div>)}
                  </div>
                  {sortedVideos.map((v,i)=>{
                    const r=ratio(v); const hot=r>=5;
                    return (
                      <div key={v.id} style={{ display:"grid", gridTemplateColumns:"1fr 55px 65px 60px 55px 36px", padding:"12px 16px", borderBottom:`1px solid ${C.border}`, gap:8, alignItems:"center", background:i===0?C.pink+"08":"transparent" }}>
                        <div>
                          <div style={{ color:C.text, fontSize:12, fontFamily:C.fontBody, fontWeight:600, marginBottom:4, lineHeight:1.3 }}>{i===0&&<span style={{ color:C.yellow, marginRight:4 }}>{Icon.star}</span>}{v.title}</div>
        ...
        {activeSub==="analysis" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>What's Working</div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    {[{label:"HOOK",data:analysis.hookAnalysis,bk:"bestHook",wk:"worstHook",c:C.cyan},{label:"FORMAT",data:analysis.typeAnalysis,bk:"bestType",wk:"worstType",c:C.green}].map(({label,data,bk,wk,c})=>(
                      <div key={label} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px" }}>
                        ...
                        <div style={{ color:C.textMed, fontSize:11, lineHeight:1.5 }}>{data.reasoning}</div>
                      </div>
                    ))}
                  </div>
                      ...
                      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                        {items?.map((item,i)=>(
                          <div key={i} style={{ display:"flex", gap:10, padding:"10px 12px", background:bg, border:`1px solid ${bc}22`, borderRadius:9 }}>
                            ...
                            <div style={{ flex:1 }}>
                              <div style={{ color:C.text, fontSize:12, fontWeight:700, fontFamily:C.fontBody, marginBottom:3 }}>{item.insight}</div>
                              <div style={{ color:C.textMed, fontSize:11, lineHeight:1.4 }}>{item.evidence}</div>
                              {item.fix && <div style={{ color:C.cyan, fontSize:11, marginTop:4, fontWeight:600 }}>→ {item.fix}</div>}
                            </div>
                          </div>
                        ))}
                      </div>
        ...
        {activeSub==="nextVids" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>Next 3 Videos</div>
                ...
                {(nextVids.tiktok||nextVids.nextVideos)?.map((v,i)=>(
                    <div key={i} style={{ background:C.card, border:`1px solid ${i===0?C.pink:C.border}`, borderRadius:14, padding:"18px 20px", borderTop:`3px solid ${i===0?C.pink:i===1?C.cyan:C.green}` }}>
                      ...
                      <div style={{ marginTop:9, padding:"9px 12px", background:C.green+"0A", border:`1px solid ${C.green}22`, borderRadius:8 }}>
                        ...
                        <div style={{ color:C.textMed, fontSize:11, lineHeight:1.5 }}>{v.whyItWillWork}</div>
                      </div>
                    </div>
                  ))}
        ...
        {activeSub==="weekly" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>Harley Brief</div>
                ...
                <>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    ...
                    <div style={{ background:C.card, border:`1px solid ${C.green}33`, borderRadius:12, padding:"14px 16px" }}>
                      ...
                      <div style={{ color:C.textMed, fontSize:11, lineHeight:1.5 }}>{weekly.topVideo?.whyItWorked}</div>
                    </div>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                      {weekly.tiktokHighlight && (
                        <div style={{ background:C.card, border:`1px solid ${C.pink}33`, borderRadius:12, padding:"14px 16px" }}>
                          ...
                          <div style={{ color:C.textMed, fontSize:11, lineHeight:1.5 }}>{weekly.tiktokHighlight.whyItWorked||weekly.topVideo?.whyItWorked}</div>
                        </div>
                      )}
                      {weekly.instagramHighlight && (
                        <div style={{ background:C.card, border:`1px solid ${C.purple}33`, borderRadius:12, padding:"14px 16px" }}>
                          ...
                          <div style={{ color:C.textMed, fontSize:11, lineHeight:1.5 }}>{weekly.instagramHighlight.whyItWorked}</div>
                        </div>
                      )}
                    </div>
                  )}
        ...
        {activeSub==="trends" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>Trend Analysis</div>
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10, marginBottom:8 }}>
                          <div style={{ flex:1 }}>
                            <div style={{ display:"flex", gap:6, alignItems:"center", marginBottom:5 }}>
                              ...
                              <Tag color={C.cyan} size={8}>{t.format}</Tag>
                            </div>
                            <div style={{ color:C.textMed, fontSize:11, lineHeight:1.5, marginBottom:8 }}>{t.why}</div>
  ...
  const TasksView = () => {
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        {/* Header */}
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>TASKS & IDEAS</div>
              ...
              {aiRecs.recommendations?.map((r,i)=>(
                <div key={i} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10 }}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ color:C.textMed, fontSize:11, lineHeight:1.5 }}>{r.why}</div>
                  </div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <div style={{ display:"flex", gap:6, background:C.card, border:`1px solid ${C.border}`, borderRadius:10, padding:"4px" }}>
                {["Bk","Harley","Both"].map(a=>(
                  <button key={a} onClick={()=>setTaskAssign(a)} style={{ padding:"4px 10px", borderRadius:7, border:"none", background:taskAssign===a?C.pink:"transparent", color:"#fff", fontFamily:C.fontBody, fontSize:10, fontWeight:700, cursor:"pointer", textTransform:"uppercase", letterSpacing:"0.05em", opacity:taskAssign===a?1:0.5 }}>{a}</button>
                ))}
              </div>
  ...
  const GrowthView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>Growth Stats</div>
  ...
  const SettingsView = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ color:C.text, fontSize:20, fontWeight:400, fontFamily:C.fontHead, letterSpacing:"0.02em" }}>Settings</div>
  ...
  function Empty({ icon, title, sub, action, actionLabel, disabled=false, color=C.pink }) {
    return (
      <div style={{ background:C.card, border:`1px dashed ${C.border}`, borderRadius:16, padding:"48px 24px", textAlign:"center" }}>
        ...
        <div style={{ color:"#fff", fontSize:13, fontWeight:700, fontFamily:C.fontBody, marginBottom:6, letterSpacing:"0.08em" }}>{title}</div>
        <div style={{ color:"#fff", fontSize:10, marginBottom:20, lineHeight:1.6, letterSpacing:"0.08em", opacity:0.6 }}>{sub}</div>
  ...
  const AddIdeaModal = () => {
    ...
    async function saveAndScore() {
      ...
      const newIdea = { id:Date.now(), ...form, notes:form.notes||"" };
      addIdea(newIdea);
      setScoring(true);
      try {
        const r = await callAI(`Score this KrapMaps app idea precisely. Return JSON only: {"appScore":0-100 (integer, precise e.g. 73 not 75),"viralityScore":0-100,"hookScore":0-100,"verdict":"one punchy sentence","viralityReason":"one sentence","hookFeedback":"one sentence","improvedHook":"better hook rewritten","bestFormat":"facecam|street|screencap","recommendations":[{"action":"specific thing to do","impact":"high|medium|low"},{"action":"specific thing to do","impact":"high|medium|low"},{"action":"specific thing to do","impact":"high|medium|low"}]}. Idea:"${newIdea.title}" Type:${newIdea.type} Hook:${newIdea.hook} Notes:${newIdea.notes||"none"}`, 1000);
        setIdeas(is=>is.map(i=>i.id===newIdea.id?{...i,aiScore:r}:i));
      } catch(e) {}
      setScoring(false);
    }
    ...
    return (
      <Modal title="Add Idea" onClose={()=>closeModal("addIdea")}>
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <Btn full onClick={saveAndScore} disabled={!form.title||scoring}>{scoring?"SCORING...":"SAVE & SCORE"}</Btn>
        </div>
      </Modal>
    );
  };
